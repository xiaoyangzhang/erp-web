...
! function(a, b) {
    ...
    g.anim = ["layui-anim", "layui-anim-01", "layui-anim-02", "layui-anim-03", "layui-anim-04", "layui-anim-05", "layui-anim-06"], f.pt.config = {
        ...
        var pageWidth = document.body.clientWidth; var pageHeight = document.body.clientHeight;
        var _width, _height;
        if( typeof f.area[0] === 'object'){
            for(var i = 0; i < f.area.length; i++){
                var maxLength = f.area[i].maxLength ? f.area[i].maxLength : 100000;
                var minLength = f.area[i].minLength ? f.area[i].minLength : 0;
                var maxHeight = f.area[i].maxHeight ? f.area[i].maxHeight : 100000;
                var minHeight = f.area[i].minHeight ? f.area[i].minHeight : 0;
                var areas = f.area[i].areas;
                if(pageWidth <= maxLength && pageWidth > minLength){
                    _width = areas[0];
                }
                if(pageHeight <= maxHeight && pageHeight > minHeight){
                    _height = areas[1];
                }
            }
        }else if(typeof f.area[0] === 'string'){
            _width = f.area[0];
            _height = f.area[1];
        }
        return f.zIndex = h, b([f.shade ? '<div class="layui-layer-shade" id="layui-layer-shade' + d + '" times="' + d + '" style="' + ("z-index:" + (h - 1) + "; background-color:" + (f.shade[1] || "#000") + "; opacity:" + (f.shade[0] || f.shade) + "; filter:alpha(opacity=" + (100 * f.shade[0] || 100 * f.shade) + ");") + '"></div>' : "", '<div class="' + g[0] + " " + (g.anim[f.shift] || "") + (" layui-layer-" + e.type[f.type]) + (0 != f.type && 2 != f.type || f.shade ? "" : " layui-layer-border") + " " + (f.skin || "") + '" id="' + g[0] + d + '" type="' + e.type[f.type] + '" times="' + d + '" showtime="' + f.time + '" conType="' + (a ? "object" : "string") + '" style="z-index: ' + h + "; width:" + _width + ";height:" + _height + (f.fix ? "" : ";position:absolute;") + '">' + (a && 2 != f.type ? "" : k) + '<div class="layui-layer-content' + (0 == f.type && -1 !== f.icon ? " layui-layer-padding" : "") + (3 == f.type ? " layui-layer-loading" + f.icon : "") + '">' + (0 == f.type && -1 !== f.icon ? '<i class="layui-layer-ico layui-layer-ico' + f.icon + '"></i>' : "") + (1 == f.type && a ? "" : f.content || "") + '</div><span class="layui-layer-setwin">' + function() {
