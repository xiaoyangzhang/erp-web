...
! function(a, b) {
    ...
    g.anim = ["layui-anim", "layui-anim-01", "layui-anim-02", "layui-anim-03", "layui-anim-04", "layui-anim-05", "layui-anim-06"], f.pt.config = {
        ...
        var a = this,
            ...
            h = b.content,
            i = "object" == typeof h;
        var pageWidth = window.screen.width; var pageHeight = window.screen.height;
        var _width = [], _height = [], index = 0;
        if( typeof b.area[0] === 'object'){
            for(var ix = 0; ix < b.area.length; ix++){
                var maxLength = parseFloat(b.area[ix].maxLength ? b.area[ix].maxLength : 100000);
                var minLength = parseFloat(b.area[ix].minLength ? b.area[ix].minLength : 0);
                var maxHeight = parseFloat(b.area[ix].maxHeight ? b.area[ix].maxHeight : 100000);
                var minHeight = parseFloat(b.area[ix].minHeight ? b.area[ix].minHeight : 0);
                var areas = b.area[ix].areas;

                _width[ix] = areas[0];
                _height[ix] = areas[1];
                if(pageWidth <= maxLength && pageWidth > minLength && pageHeight <= maxHeight && pageHeight > minHeight){
                    index = ix;
                }
            }
            b.area = [ _width[index], _height[index] ];
        }
        //else if(typeof b.area[0] === 'string'){
        //    "string" == typeof b.area && (b.area = "auto" === b.area ? ["", ""] : [b.area, ""]);
        //
        //}
        ...
        switch (f.type) {
            case 2:
                ...
                break;
            default:
                "" === f.area[1] ? f.fix && i[1] > window.screen.height && (i[1] = window.screen.height, b("." + g[5])) : b("." + g[5])
        }
        return e
        ...
        a.offsetTop = (window.screen.height - e[1]) / 2, a.offsetLeft = (window.screen.width - e[0]) / 2, f ? (a.offsetTop = b.offset[0], a.offsetLeft = b.offset[1] || a.offsetLeft) : "auto" !== b.offset && (a.offsetTop = b.offset, "rb" === b.offset && (a.offsetTop = window.screen.height - e[1], a.offsetLeft = window.screen.width - e[0])), b.fix || (a.offsetTop = /%$/.test(a.offsetTop) ? window.screen.height * parseFloat(a.offsetTop) / 100 : parseFloat(a.offsetTop), a.offsetLeft = /%$/.test(a.offsetLeft) ? window.screen.width * parseFloat(a.offsetLeft) / 100 : parseFloat(a.offsetLeft), a.offsetTop += d.scrollTop(), a.offsetLeft += d.scrollLeft()), c.css({
            top: a.offsetTop - 100,// edit by zzy cause of the iframe left and top
            left: a.offsetLeft - 140 // edit by zzy cause of the iframe left and top
        })
    }, f.pt.tips = function() {
        var a = this,
            b = a.config,
        ...
        b.tips[1] || j.remove(), i.autoLeft = function() {
            i.left + f[0] - window.screen.width > 0 ? (i.tipLeft = i.left + i.width - f[0], j.css({
                right: 12,
                left: "auto"
            })) : i.tipLeft = i.left
        }, i.where = [function() {
        ...
        }], i.where[k - 1](), 1 === k ? i.top - (d.scrollTop() + f[1] + 16) < 0 && i.where[2]() : 2 === k ? window.screen.width - (i.left + i.width + f[0] + 16) > 0 || i.where[3]() : 3 === k ? i.top - d.scrollTop() + i.height + f[1] + 16 - window.screen.height > 0 && i.where[0]() : 4 === k && f[0] + 16 - i.left > 0 && i.where[1](), e.find("." + g[5]).css({
        ...
        return b.move && f.attr("move", "ok"), f.css({
            ...
            if (e.ismove) {
                ...
                if (a.preventDefault(), !b.moveOut) {
                    e.setY = d.scrollTop();
                    var g = window.screen.width - e.move.outerWidth(),
                        h = e.setY;
                    0 > c && (c = 0), c > g && (c = g), h > f && (f = h), f > window.screen.height - e.move.outerHeight() + e.setY && (f = window.screen.height - e.move.outerHeight() + e.setY)
                }
                e.move.css({
                    left: c,
                    top: f
