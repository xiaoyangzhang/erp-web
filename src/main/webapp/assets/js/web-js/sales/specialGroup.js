				...
				success : function(data) {
					if (data.success) {
						$.success('操作成功',function(){
							refreshWindow('编辑一地散订单','toEditSpecialGroup.htm?id='+data.groupId)
						});
					} else {
						$.error(data.msg);
...
function insertGroupOnly(tid){
	...
	layer.open({ 
		...
		area : [ '900px', '500px' ],
		shadeClose : true,
		content : '../specialGroup/getSpecialGroup.do?tid='+tid,
		btn: ['确定', '取消'],
...
function editGroup(id){
	newWindow('编辑一地散订单','specialGroup/toEditSpecialGroup.htm?id='+id);
}
/**
 *  删除订单
 * @param id
