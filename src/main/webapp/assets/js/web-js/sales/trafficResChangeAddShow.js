var addGuest=function(str){
	...
	if($("#orderId").length>0){ //团队
		...
		if((count+1)>(Number(numAdult)+Number(numChild)+Number(numChildBaby))){
			$.warn("超过该订单最大容纳人数！");
			return ;
		}
...
var addTran = function (str,rowObj){
	$("#"+str+"BtnState").css("display","");
	...
	$("#"+str+"Data").append(html);
	  ...
	  var arrivalCity =  $("input[name='groupOrderTransportList["+count+"].arrivalCity']");
			...
			// console.log(rowObj);
			if (rowObj){
				if(rowObj.departureCity){
					$("input[name='groupOrderTransportList["+count+"].departureCity']").val(rowObj.departureCity);  //出发
					$("input[name='groupOrderTransportList["+count+"].arrivalCity']").val(rowObj.arrivalCity);
					$("input[name='groupOrderTransportList["+count+"].classNo']").val(rowObj.classNo);
					$("input[name='groupOrderTransportList["+count+"].departureDate']").val(rowObj.departureDate);
					$("input[name='groupOrderTransportList["+count+"].departureTime']").val(rowObj.departureTime);
					// priceRowCals(count);
	...
	//type 项目下拉框值，　str=newPrice, rowObj：json对象（格式与新增的行对应）
		...
		var numPerson =  $("input[name='groupOrderPriceList["+count+"].numPerson']");
				...
				if (rowObj){
					...
					if (rowObj.unitPrice){ 
						$("input[name='groupOrderPriceList["+count+"].numPerson']").val(rowObj.numPerson).attr("row", count).attr("remark", rowObj.itemName);
						...
						$("input[name='groupOrderPriceList["+count+"].itemName']").val(rowObj.itemName);
						$("select[name='groupOrderPriceList["+count+"].itemId']").find("option[text='"+rowObj.itemName+"']").attr("selected",true);
						if (rowObj.priceLockState != '0')
							$("input[name='groupOrderPriceList["+count+"].unitPrice']").closest("tr").find("a[lang='aDeletePriceRow']").css("display", "none");
...
var priceRowIsExists = function(remark){
	var exists = -1;
	$("#newPriceData input[name*='.numPerson']").each(function(){
		if ($(this).attr("remark") == remark){
			exists = $(this).attr("row");
		}
	});
	return exists;
}
