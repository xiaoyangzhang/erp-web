 ...
 * @constructor
 */
var SalesRoute = function SalesRoute(dataConstruct){
    ...
    $content.delegate('.day_delete', 'click', function(){
        ...
        $content.children('tr').each(function(i){
            $(this).find('.day_sequence').html(SalesRoute.timeByDateCalculate(i + 1));
...
SalesRoute.prototype.dayAdd = function(data){
...
SalesRoute.prototype.trafficAdd = function(dayIndex, trafficIndex, data){
    ...
    var text = template('traffic_template', {dayNum : dayIndex,id:data.id, trafficImgPath : typePath,
...
SalesRoute.prototype.supplierAdd = function(dayIndex, supplierIndex, data){
    ...
    var text = template('supplier_template', {dayNum : dayIndex, id:data.id,supplierIndex : supplierIndex,
...
SalesRoute.prototype.imgAdd = function(dayIndex, imgIndex, data){
    //{dayNum : dayIndex, imgIndex : imgIndex + i, imgSrc : arr[i].thumb, imgName : arr[i].name, imgPath : arr[i].path}
    var text = template('img_template', {dayNum : dayIndex,id:data.id, imgIndex : imgIndex, imgSrc : data.thumb, imgName : data.name, imgPath : data.path});
    $('#tr_day_' + dayIndex + ' .img_details').append(text);
};

SalesRoute.prototype.trafficAddEvent = function(el){
    var self = this;
    var win;
...
SalesRoute.prototype.supplierAddEvent = function(el){
    var self = this;
    var dayIndex = $(el).closest('tr').attr('id').substring(7);
...
SalesRoute.prototype.imgAddEvent = function(el){
    var self = this;
    var dayIndex = $(el).closest('tr').attr('id').substring(7);
...
SalesRoute.timeByDateCalculate = function(addDate){
    var result = new Date(startDate + (addDate - 1) * 86400000);
    return result.getFullYear() + "-" + (result.getMonth() + 1) + "-" + result.getDate();
};

template.helper('timeByDateCalculate', function (date, addDate) {
    return SalesRoute.timeByDateCalculate(date, addDate);
});