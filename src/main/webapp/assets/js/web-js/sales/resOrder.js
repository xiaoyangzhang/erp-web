					...
					if (data.success) {
						$.success('操作成功',function(){
							if(data.see=="1"){
							 ...
							 refreshWindow('编辑订单','editResOrder.htm?id='+data.groupId)
							}
						});
					} else {
						$.error(data.msg);
...
/*	if(isNaN(numAdult) || isNaN(numChild) || isNaN(numChildBaby)){
	...
	if((Number(count)+Number(sum))>(Number(numAdult)+Number(numChild)+Number(numChildBaby))){
		$.warn("超过该订单最大容纳人数！");
		return ;
	}
