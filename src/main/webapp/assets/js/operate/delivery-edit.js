		...
		$("#bookingForm").validate({
				...
				submitHandler : function(form) {
					...
					$("#priceTable tbody tr").each(function(){
						booking.priceList.push({id:$(this).find("input[name='priceId']").val(),itemName:$(this).find("select[name='itemName']").val(),remark:$(this).find("textarea[name='note']").val(),unitPrice:$(this).find("input[name='unitPrice']").val(),numTimes:$(this).find("input[name='numTimes']").val(),numPerson:$(this).find("input[name='numPerson']").val(),totalPrice:$(this).find("input[name='totalPrice']").val()});
								...
								$.ajax({
				   			        ...
				   			        success: function(data) {
				   			            if (data.sucess == true) {
							            	...
							            	refreshWindow("修改下接社订单","../booking/delivery.htm?gid="+data['groupId']+"&bid="+data['bookingId']);
				   			            	});
...
var price = {
	...
	bindEvent:function(){
		$("#priceTable tbody tr").each(function(){
			...
			//绑定行计算
			var unitPriceObj=$(this).find("input[name='unitPrice']");
			var numTimesObj=$(this).find("input[name='numTimes']");
			var numPersonObj=$(this).find("input[name='numPerson']");
			var totalPriceObj=$(this).find("input[name='totalPrice']");
