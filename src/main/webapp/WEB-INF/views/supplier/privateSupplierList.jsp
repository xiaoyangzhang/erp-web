			...
			<table class="w_table" cellspacing="0" cellpadding="0">
				...
				<c:forEach items="${page.result}" var="supplierInfo"
					...
					<tr >
						...
						<c:if test="${supplierInfo.supplierType==2 }">餐厅</c:if><c:if test="${supplierInfo.supplierType==3 }">酒店</c:if><c:if test="${supplierInfo.supplierType==4 }">车队</c:if><c:if test="${supplierInfo.supplierType==5 }">景区</c:if><c:if test="${supplierInfo.supplierType==6 }">购物</c:if><c:if test="${supplierInfo.supplierType==7 }">娱乐</c:if><c:if test="${supplierInfo.supplierType==8 }">导游</c:if><c:if test="${supplierInfo.supplierType==9 }">机票代理</c:if><c:if test="${supplierInfo.supplierType==10 }">火车票代理</c:if><c:if test="${supplierInfo.supplierType==11 }">高尔夫</c:if><c:if test="${supplierInfo.supplierType==12 }">其他</c:if><c:if test="${supplierInfo.supplierType==15 }">保险</c:if>详情', '<%=ctx%>/supplier/toEditSupplier.htm?id=${supplierInfo.id}&operType=0')"
