...
<body>
	<div class="p_container">
		...
		<div class="p_container_sub pl-10 pr-10">
			<form id="searchSupplierForm" action="toSuplierList.htm" method="post">
			<dl style="margin-top:5px;">
					...
					<dd class="inl-bl">
						<div class="dd_left">分类:</div>
						<div class="dd_right grey">
							<input type="text" name="className" id="className" value="${supplierInfo.className }" class="input-small" />
						</div>
						<div class="clear"></div>
					</dd>
			</dl>
			<dl style="margin:5px 0px;">
					<dd class="inl-bl">
						<div class="dd_left">法人:</div>
						<div class="dd_right grey">
							<input type="text" name="lawPerson" id="lawPerson" value="${supplierInfo.lawPerson }" class="input-small" />
						</div>
						<div class="clear"></div>
					</dd>
					<dd class="inl-bl">
			...
			<table class="w_table" cellspacing="0" cellpadding="0">
				...
				<thead>
					<tr>
						...
						<th>编码<i class="w_table_split"></i></th>
						<th>类别<i class="w_table_split"></i></th>
						<th>分类<i class="w_table_split"></i></th>
				...
				<c:forEach items="${page.result}" var="supplierInfo"
					...
					<tr >
						...
						<td style="text-align: left;">${supplierInfo.code}</td>
						...
						<td><a class="def" onclick="newWindow('<c:if test="${supplierInfo.supplierType==1 }">组团社</c:if><c:if test="${supplierInfo.supplierType==2 }">餐厅</c:if><c:if test="${supplierInfo.supplierType==3 }">酒店</c:if><c:if test="${supplierInfo.supplierType==4 }">车队</c:if><c:if test="${supplierInfo.supplierType==5 }">景区</c:if><c:if test="${supplierInfo.supplierType==6 }">购物</c:if><c:if test="${supplierInfo.supplierType==7 }">娱乐</c:if><c:if test="${supplierInfo.supplierType==8 }">导游</c:if><c:if test="${supplierInfo.supplierType==9 }">机票代理</c:if><c:if test="${supplierInfo.supplierType==10 }">火车票代理</c:if><c:if test="${supplierInfo.supplierType==11 }">高尔夫</c:if><c:if test="${supplierInfo.supplierType==12 }">其他</c:if><c:if test="${supplierInfo.supplierType==15 }">保险</c:if>详情', '<%=ctx%>/supplier/toEditSupplier.htm?id=${supplierInfo.id}&operType=0')" href="javascript:void(0)">查看</a> 
							<c:if test="${optMap['EDIT'] }">
								...
								<c:choose>
									<c:when test="${supplierInfo.supplierType ==4 }">
										<a class="def" onclick="newWindow('协议车队 ', '<%=ctx%>/contract/'+${supplierInfo.id}+'/fleet-list.htm')" href="javascript:void(0)">协议</a>
									</c:when>
									<c:otherwise>
