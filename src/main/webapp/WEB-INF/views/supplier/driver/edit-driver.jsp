...
<body>
	<div class="p_container">
		<form class="form-horizontal" id="driverForm">
			...
			<p class="p_paragraph_title"><b>司机基本信息:</b></p>
			<dl class="p_paragraph_content">
	    		<dd>
	    			<div class="dd_left"><i class="red">* </i>姓名：</div> 
	    			<div class="dd_right">
	    				<input name="name" type="text" class="IptText300" value="${driver.name }" />
	    			</div>
					<div class="clear"></div>
	    		</dd> 
	    		<dd>
	    			...
	    			<div class="dd_right">
	    				...
	    				<c:if test="${not empty driver.id ne ''}">
	    					<input name="gender" type="radio" value="0" <c:if test="${driver.gender eq 0 }">checked</c:if> />男
	    					<input name="gender" type="radio" value="1" <c:if test="${driver.gender eq 1 }">checked</c:if> />女
	    				</c:if>
					</div>
					<div class="clear"></div>
	    		</dd>
	    		<dd>
	    			...
	    			<div class="dd_right">
	    				<input type="text" name="birthDate" class="Wdate" value="<fmt:formatDate pattern='yyyy-MM-dd' value='${driver.birthDate }'/>"
	    		...
	    		<dd>
	    			...
	    			<div class="dd_right">
	    				<input type="hidden" name="nationalityName" id="nationalityName" value="${driver.nationalityName }"/>
	    				<select name="nationality" id="nationality">
	    					<option value="">请选择</option>
	    					<c:forEach var="mz" items="${mzList}">
		    					<option value="${mz.id }" <c:if test="${driver.nationality!=null && mz.id == driver.nationality }">selected</c:if>>${mz.value }</option>	    						
	    					</c:forEach>
	    				</select>
					</div>
					<div class="clear"></div>
	    		...
	    		<dd>
	    			...
	    			<div class="dd_right">
	    				<input name="nativePlace" type="text" class="IptText300" value="${driver.nativePlace }" />
					</div>
					<div class="clear"></div>
	    		</dd>
	    		<dd>
	    			...
	    			<div class="dd_right">
	    				<input name="mobile" type="text" class="IptText300" value="${driver.mobile }" />
					</div>
					<div class="clear"></div>
	    		</dd>
	    		...
	    		<dd>
	    			...
	    			<div class="dd_right">
	    				<input name="idCardNo" type="text" class="IptText300"  value="${driver.idCardNo }">
					</div>
					<div class="clear"></div>
	    		</dd>
	    		<dd>
	    			...
	    			<div class="dd_right">
	    				<input type="text" name="licenseDate" class="Wdate"
							onClick="WdatePicker({dateFmt:'yyyy-MM-dd'})" value="<fmt:formatDate pattern='yyyy-MM-dd' value='${driver.licenseDate }'/>"/>
					</div>
					<div class="clear"></div>
	    		</dd>
				<dd>
	    			...
	    			<div class="dd_right">
	    				<input name="licenseNo" type="text" class="IptText300" value="${driver.licenseNo }">
					</div>
					<div class="clear"></div>
	    		</dd>
				<dd>
	    			<div class="dd_left">准驾车型：</div> 
	    			<div class="dd_right">
	    				<input id="permitCarType" name="permitCarType" type="hidden" class="IptText300" value="${driver.permitCarType }">
	    				<c:forEach items="${carList }" var="car">
		    				<input name="ckb_permitCarType" onchange="carChange(this)" type="checkbox" value="${car.value }">${car.value }
	    				</c:forEach>
					</div>
					<div class="clear"></div>
	    		</dd>
				<dd>
	    			<div class="dd_left">档案号：</div> 
	    			<div class="dd_right">
	    				<input name="licenseNoDoc" type="text" class="IptText300" value="${driver.licenseNoDoc }">
					</div>
					<div class="clear"></div>
	    		</dd>
				<dd>
	    			<div class="dd_left">专职/兼职：</div>
	    			<div class="dd_right">
    				<select name="isFullTime">
    					<option value="1" <c:if test="${driver.isFullTime eq 1 }">selected</c:if>>专职</option>
    					<option value="2" <c:if test="${driver.isFullTime eq 2 }">selected</c:if>>兼职</option>
    				</select>
					</div>
					<div class="clear"></div>
	    		</dd>
	    		<dd>
	    			...
	    			<div class="dd_right">
	    				<input type="hidden" name="provinceName" id="provinceName" value="${driver.provinceName}"/>
	    				<input type="hidden" name="cityName" id="cityName" value="${driver.cityName}" />
	    				<input type="hidden" name="areaName" id="areaName" value="${driver.areaName}" />
	    				<input type="hidden" name="townName" id="townName" value="${driver.townName}" />
	    				<select name="provinceId" id="provinceId" class="input-small">
							<option value="">请选择省</option>		
							<c:forEach items="${allProvince}" var="province">
								<option value="${province.id }" <c:if test="${driver.provinceId!=null && driver.provinceId==province.id  }">selected</c:if> >${province.name }</option>
							</c:forEach>					
						</select> <select name="cityId" id="cityId" class="input-small">
							<option value="">请选择市</option>
							<c:forEach items="${cityList}" var="city">
								<option value="${city.id }" <c:if test="${driver.cityId!=null && city.id==driver.cityId }"> selected</c:if>>${city.name }</option>
							</c:forEach>
						</select> <select name="areaId" id="areaId" class="input-small">
							<option value="">请选择区县</option>
							<c:forEach items="${areaList}" var="area">
								<option value="${area.id }"
									<c:if test="${driver.areaId!=null && area.id==driver.areaId }"> selected="selected" </c:if>>${area.name }</option>
							</c:forEach>
						</select> <select name="townId" id="townId" class="input-small">
							<option value="">请选择街道</option>
								<c:forEach items="${townList}" var="town">
								<option value="${town.id }"
									<c:if test="${driver.townId!=null && town.id==driver.townId }"> selected="selected" </c:if>>${town.name }</option>
							</c:forEach>
						</select>
						<input name="addr" type="text" class="IptText300" value="${driver.addr }"/>
					</div>
					<div class="clear"></div>
				</dd>
	    		<dd>
