...
<script type="text/javascript">
	...
	function getCars(){
		...
		$("input[name='supplierCarChecked']:checked").each(function() {// 遍历每一个名字为interest的复选框，其中选中的执行函数
			carArray.push($(this).val());// 将选中的值添加到数组chk_value中
		});
		return carArray;
	}   
   /*  $(function() {
...
<body>
	<div class="p_container">
		<form id="searchSupplierCarForm" action="toAllSupplierCarList.htm"
			method="post">
				...
				<div class="searchRow">
					<ul>
						...
						<li><input type="text" name="seatStart"
							value="${supplierCar.seatStart}" />~<input type="text"
							name="seatEnd" value="${supplierCar.seatEnd}" /></li>
						<li class="text" style="width: 76px;">车牌号:</li>
						<li><input type="text" name="carLisenseNo"
							value="${supplierCar.carLisenseNo }" class="input-small" /></li>
						...
						<li>
							<button class="button button-primary button-small"
								onclick="searchBtn();">搜索</button> <c:if
								test="${optMap['EDIT'] }">
								<a class="button button-primary button-small"
									onclick="newWindow('新增车辆档案','supplierCar/toAddSupplierCar.htm')"
		...
		<table class="w_table" cellspacing="0" cellpadding="0">
			...
			<c:forEach items="${voList}" var="vo" varStatus="status">
				<tr>
					<td><input type="checkbox" name="supplierCarChecked"
						value="${vo.supplierCar.id}" /></td>
					<td>${status.count}</td>
					...
					<td><fmt:formatDate value="${vo.supplierCar.buyDate }"
							pattern="yyyy-MM-dd" /></td>
					<td><fmt:formatDate value="${vo.supplierCar.examDate }"
							pattern="yyyy-MM-dd" /></td>
					<!-- 					<td> -->
