	...
	<c:forEach items="${pageBean.result}" var="guide" varStatus="status">
		<div class="guide_detail">
			<div class="guide_butt">
				<button onclick="newWindow('修改导游','<%=staticPath %>/supplier/editGuide.htm?id=${guide.id }')" class="button button-rounded button-tiny">修改</button>
				<button onclick="del(${guide.id })" class="button button-rounded button-tiny">删除</button>
			</div>
			<img class="guide_portrait"
				src="<c:if test="${empty guide.photo}"><%=staticPath%>/assets/css/supplier/img/linshi.png</c:if> <c:if test="${!empty guide.photo}"> ${images_source}${guide.photo}</c:if>" />
			<div class="guide_info">
				...
				<div class="guide_msg mt-10">
					...
					<p class="w-200">开户行：${guide.bankName }</p>
					<p class="w-200">银行账号：${guide.bankAccount }</p>
				</div>
