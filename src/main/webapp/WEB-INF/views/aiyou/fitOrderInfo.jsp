...
<script type="text/javascript">
		...
		$(function(){
		    ...
		    // 自动计算身份证；
		    for (var i=0; i<$(".certificateNum").length; i++){
		    	recCertifNum(i);
		    }
		    $("textarea[name='groupOrder.remarkInternal']").focus();
		});
</script>
				...
				<table border="0" cellspacing="0" cellpadding="0" class="searchTab">
					...
					<tr>
						...
						<td>
							...
							<input type="hidden" name="groupOrder.aiyouGroupId" value="${vo.groupOrder.aiyouGroupId }"/>
						</td>
					</tr>
					<tr>
						<td>组团社联系人：</td>
				...
				<dl class="p_paragraph_content">
					...
					<dd>
						...
						<div class="dd_right">
							...
							<p><font color="red">请根据爱游的备注信息，核实后保存。</font>
						</div>
...
<div id="selectSupplier" style="display: none">
	...
	function selectSupplierBtn() {
		...
		var optionHtml="";
		$.getJSON("/fitOrder/getGroupPirceData.do?productId=" + $("#productId").val()+"&supplierId="+$("#supplierId").val()+"&date="+$("#departureDate").val(), function(data) {
		...
		if (optionHtml == ""){
			optionHtml+="<option value='0,0,0,0'>无</option>";
		}
