...
<body>
	<div class="p_container">
		...
		<div class="p_container_sub">
			<form id="saveAgencyFitGroupInfoForm">
										...
										<td>
											...
											<c:if test="${operType==1}">
												<c:if test="${fitGroupInfoVO.tourGroup.groupState!=3 and fitGroupInfoVO.tourGroup.groupState!=4}">
													...
													<c:if test="${groupOrder.stateFinance!=1}">
													<a href="javascript:void(0);"onclick="delGroupOrder(${groupOrder.id})" class="def">删除</a>
													</c:if>
											    </c:if>
										  	</c:if>
										</td>
				...
				<!--  已审核+已封存不能对订单操作-->
					...
					<div class="Footer" style="position:fixed;bottom:0px; right:0px; background-color: rgba(58,128,128,0.7);width: 100%;padding-bottom:0px;margin-bottom:0px;text-align: center;">
						...
						<button type="button" class="button button-primary button-small" onclick="printAgencyInfo()">打印</button>
					</div>
		</form>
		</div>
	</div>
	  <%@ include file="../../sales/template/groupRouteTemplate.jsp"%>
</body>
</html>
<div style="display: none; text-align: center; margin-top: 10px" id="exportInfo">
	<div style="margin-top: 10px">
		<a href="../agencyFitGroup/toGroupNoticePreview.htm?groupId=${fitGroupInfoVO.tourGroup.id}" target="_blank" id="cttz" class="button button-primary button-small">出团通知</a>
	</div>
	<div style="margin-top: 10px">
