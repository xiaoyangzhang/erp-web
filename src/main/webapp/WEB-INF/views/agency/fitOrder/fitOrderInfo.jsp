	...
	<div class="p_container">
		<div class="p_container_sub" id="tab1">
			<form id="fitOrderForm">
				...
				<table border="0" cellspacing="0" cellpadding="0" class="searchTab">
					...
					<tr>
						...
						<td>
							...
							<input type="hidden"  name="groupOrder.priceId" id="priceId" value="${vo.groupOrder.priceId}" />
							<input type="hidden"  id="groupId" value="${groupId}" />
							<input type="hidden"  id="adultPrice" value="${adultPrice}" />
							<input type="hidden"  id="childPrice" value="${childPrice}" />
							<input type="hidden"  id="adultCost" value="${adultCost}" />
							<input type="hidden"  id="childCost" value="${childCost}" />
						...
						<td>订单类型:</td>
						<td>
							<input type="hidden" name="groupOrder.type" class="IptText300"  value="${vo.groupOrder.type}" />
							<c:if test="${vo.groupOrder.type==1}">确认订单</c:if><c:if test="${vo.groupOrder.type==0}">预留订单</c:if>
						</td>
					</tr>
					<tr>
						<td><i class="red">* </i>销售 :</td>
						<td>
							${vo.groupOrder.saleOperatorName}
							<input name="groupOrder.saleOperatorName" id="saleOperatorName" value="${vo.groupOrder.saleOperatorName}" type="hidden">
							<input type="hidden" name="groupOrder.saleOperatorId" id="saleOperatorId" value="${vo.groupOrder.saleOperatorId}" /></td>
						<td><i class="red">* </i>计调:</td>
						<td>
							${vo.groupOrder.operatorName}
							<input name="groupOrder.operatorName" id="operatorName"  value="${vo.groupOrder.operatorName}" type="hidden"> 
							<input type="hidden" name="groupOrder.operatorId" id="operatorId" value="${vo.groupOrder.operatorId}"></td>
					</tr>
					<tr>
						<td><i class="red">* </i>客户：</td>
						<td>
						<c:if test="${editType }">
							<input name="groupOrder.supplierId" id="supplierId" type="hidden" value="${vo.groupOrder.supplierId }" />
							<input name="groupOrder.supplierName" id="supplierName" type="text" class="IptText300"  value="${vo.groupOrder.supplierName }" />
						...
						<td>客户单号：</td>
						<td>
							<input type="text" name="groupOrder.supplierCode" class="IptText300" placeholder="客户单号" value="${vo.groupOrder.supplierCode }" />
						</td>
					</tr>
					<tr>
						<td>联系人/介绍人：</td>
					...
					<tr>
						...
						<td>
							<input type="text" name="groupOrder.receiveMode" class="IptText300" placeholder="客人姓名、电话" value="${vo.groupOrder.receiveMode}" />
							
						<td>合同编号：</td>
						<td>
							<input type="text" name="groupOrder.contractNo" class="IptText300" placeholder="合同编号" value="${vo.groupOrder.contractNo}" />
						</td>
					</tr>
					<tr>
					...
					<tr>
						<td>客人来源：</td>
						<td>
							<input type="hidden" name="groupOrder.guestSourceName" class="IptText300" id="guestSourceName" value="${vo.groupOrder.guestSourceName }" />
							<select name="groupOrder.guestSourceId" id="guestSourceId">
								<option value="-1">请选择</option>
								<c:forEach items="${guestSourceList}" var="source">
									<option value="${source.id }"
										<c:if test="${source.id==vo.groupOrder.guestSourceId }"> selected="selected" </c:if>>${source.value}</option>
								</c:forEach>
							</select>
						</td>
						<td><i class="red">* </i>团人数：</td>
						<td>
							<input style="width: 40px;" type="text" name="groupOrder.numAdult" placeholder="成人数" value="${(empty vo.groupOrder.numAdult)?0:vo.groupOrder.numAdult}" onblur="changeNum()" />
							~
							<input style="width: 40px;" type="text" name="groupOrder.numChild" placeholder="小孩数" value="${(empty vo.groupOrder.numChild)?0:vo.groupOrder.numChild}" onblur="changeNum()"/>
							...
							<table cellspacing="0" cellpadding="0" class="w_table">
								<thead>
									<tr>
										...
										<th width="17%">备注<i class="w_table_split"></i></th>
									</tr>
								</thead>
								<tbody>
									<tr>
										...
										<td><input name="hotelInfo.remark" type="text"
											style="width: 100px;"
											value="${vo.hotelInfo.remark}" />
										</td>
								...
								<tbody id="newPriceData">
									<c:forEach items="${vo.groupOrderPriceList }" var="price"
										...
										<tr>
											...
											<td><input style="width: 80%;<c:if test="${price.priceLockState==1 or price.stateFinance==1}">border-color: red </c:if>" <c:if test="${price.stateFinance==1 }">readonly="readonly" </c:if>  type="text"
											...
											<c:if test="${operType==1}"><td><c:if test="${price.priceLockState!=1 or price.stateFinance==0 }"><a href="javascript:void(0);"
												onclick="delPriceTable(this,'newPrice')">删除</a></c:if></td></c:if>
										</tr>
							...
							<table cellspacing="0" cellpadding="0" class="w_table">
								<thead>
									<tr>
										...
										<th width="8%">付款时间<i class="w_table_split"></i></th>
				...
				<div style="display: <c:if test="${isSales}">none</c:if>">
				<p class="p_paragraph_title">
					<b>预算成本</b>
				</p>
				<dl class="p_paragraph_content">
				...
				<div class="Footer" style="position:fixed;bottom:0px; right:0px; background-color: rgba(58,128,128,0.7);width:100%;padding-bottom:0px;margin-bottom:0px;  text-align: center;">
						...
						<c:if test="${!editType and  !isSales }">
							<button type="button" class="button button-primary button-small" onclick="lockOrUnLock(${vo.groupOrder.id},${vo.groupOrder.orderLockState})"><c:if test="${vo.groupOrder.orderLockState==0 }">锁单</c:if><c:if test="${vo.groupOrder.orderLockState==1 }">取消锁单</c:if></button>
						</c:if>
				</div>
				</c:if>
			</form>
...
<div id="selectSupplier" style="display: none">
	...
	var supplierNameComplete={
			  source: function( request, response ) {
				  ...
				  $.ajax({
					  type : "post",
					  url : "<%=path%>/tourGroup/getPriceSupplierNameForAgency",
					  data : {
						  productId: productId,
						  groupId:groupId,
						  priceId:priceId,
					  ...
					  dataType : "json",
					  success : function(data){
						  if(data && data.success == 'true' && data.type==0){
							  response($.map(data.result,function(v){
								  return {label:v.supplierName,supplierId:v.supplierId};
							  }));
						  }
						  if(data && data.success == 'true' && data.type==1){
							  response($.map(data.result,function(v){
								  return {label:v.nameFull,supplierId:v.id};
							  }));
						  }
					  },
					  error : function(data,msg){}
