...
<script type="text/javascript">	
	$(function(){
		...
		var setMonthData=function(year,month){
			YM.post('stockMonth.do',{stockId:'${stockId}',year:year,month:month},function(result){
				if(!result){
					return;
				}
				...
				$("input[name='dateId']").val('');
				$("input[tag='stock']").val('');
				$(".w_table>tbody>tr>td[tag='receive']").html('');
				for (var i = 0; i < data.length; i++) {
	                var item = data[i];
	                var itemDate = new Date(item.stockDate);
		...
		$.validator.addMethod("reasonable", function(value,element) {
		          var receive = $(element).attr("receive"), total =  parseInt($(element).attr("total"));
		          var receiveNum = parseInt(receive);
		          value = isNum(value)?value:0;
		          var validNum = total + parseInt(value);
		          return this.optional(element) || (validNum>=receiveNum);
		}, "计划数应大于已收数");
		
		$.validator.setDefaults({
			submitHandler : function(form) {
				var stockArr = onData();
				YM.post('saveStock.do',{stockId:'${stockId}',stockStr:JSON.stringify(stockArr),year:yearSel,month:monthSel},function(){
	    ...
	    var addValidateRule = function(datestr){
			if(datestr){
				...
				$("#stock-"+datestr).closest("tr").find("input[name='subbox']").trigger("change");
			}else{
				$("table.w_table>tbody").find("input[name='subbox']").each(function(){
					...
					if($(this).attr("checked")){
						stock.rules("add",{required:true,reasonable:true,messages: { required: "必填"}});	
					}
					$(this).trigger("change");
				})
			}
		...
		$("#checkAll").click(function(){
        	$("input[name='subbox']").not("input[disabled='disabled']").attr("checked",this.checked).trigger("change");;
	...
	function stocklog(stockDateId,stockDate){
		var dateString =stockDate.toString();
		var pattern = /(\d{4})(\d{2})(\d{2})/;
		var formatedDate = dateString.replace(pattern, '$1-$2-$3');
		newWindow("库存调整明细","taobaoProect/stockLogDetails.do?stockDateId="+stockDateId+ "&stockDate=" + formatedDate);
	}
</script>
</html>
