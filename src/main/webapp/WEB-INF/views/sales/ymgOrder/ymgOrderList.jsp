				...
				<form method="post" id="specialGroupListForm">
					...
					<li class="text">部门:</li>
						...
						<li style="padding-left:10px">
							...
							<button type="button" onclick="newWindow('新增团订单','ymg/addYMGOrder.htm')" class="button button-primary button-small">新增订单</button>
...
function toOperatorExcel(){
	$("#toOperatorExcelId").attr("href","<%=staticPath%>/taobao/toOrderPreview.htm?startTime="+$("#startTime").val()
			+"&endTime="+$("#endTime").val()
...
var opGrid = {
    ...
    loadGrid: function(){
        ...
        $("#tableDiv").jqGrid({
            ...
            colNames: ['团号', '出发日期', '产品名称', '平台来源', '客人', '成人','儿童','全陪','金额','类别','业务','流程','团状态','销售员','计调员','操作'],
            colModel: [
                ...
                {name: 'type',index: 'type',width: 80, align: "center", sortable: false,
                    formatter:function(value,options,rowData){
                        if( value==0 ){return '预留';}
                        if( value==1 ){return '确认';}
                    }
                },
                ...
                {name: 'orderLockState',index: 'orderLockState',width: 80, align: "center", sortable: false,
                    formatter:function(value,options,rowData){
                        ...
                        if( value==3 ){return "<span style='color:green'>"+'已锁单'+"</span>";}
                    }
                },
    ...
    getParam: function(){
			...
			var params = {'byType':$("#byType").val(), 'dateType':$("#dateType").val(), 'startTime':$("#startTime").val()
					...
					,'guestName':$("#guestName").val(),'mobile':$("#mobile").val(),'type':$("#type").val()
    ...
    formatOptions:function (cellValue, options, rowObject){
			var ops =  '<div class="tab-operate">' +
				...
				if((rowObject.orderLockState != 3) && rowObject.stateFinance !=1) {
					ops+='<a href="javascript:void(0);" onclick="newWindow(\'编辑订单\',\'ymg/toEditYMGOrder.htm?id='+rowObject.id+'&see=1\')" class="def">编辑</a>';
				}else{
					ops+='<a href="javascript:void(0);" onclick="newWindow(\'查看订单\',\'ymg/toEditYMGOrder.htm?id='+rowObject.id+'+&see=0\')"  class="def" >查看</a>';
				}
				if(rowObject.orderLockState == 0 && rowObject.type == 1) {
					ops+='<a href="javascript:void(0);" onclick="changeorderLockState('+rowObject.id+')" class="def">提交给计调</a>';
				}
				if(rowObject.orderLockState != 3 && rowObject.stateFinance !=1 && rowObject.totalCash=='0.0000' && rowObject.groupState!=3 && rowObject.groupState!=4) {
			...
			getPageFooter: function(totalRow){
				var pageObj = {sumTotal:0,pAdult:0, pChild:0, pGuide:0};
				  ...
				  var options = {
						url:"ymgOrderList_PostFooter.do",
				    	type:"post",
				    	dataType:"json",
