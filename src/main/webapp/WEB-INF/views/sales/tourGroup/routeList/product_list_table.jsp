...
<table cellpadding="0" class="w_table" > 
    <thead>
    	<tr>
    		...
    		<th>线路类型<i class="w_table_split"></i></th> -->
    		<th>计调</th>
    ...
    <c:forEach items="${page.result}" var="productInfo" varStatus="status">
       <tr id="${productInfo.id }"> 
          ...
          <%-- <td width="15%">${productInfo.destProvinceName }/${productInfo.destCityName }</td>
          <td width="15%"><c:if test="${productInfo.type eq 1}">国内长线</c:if><c:if test="${productInfo.type eq 2} ">周边短线</c:if><c:if test="${productInfo.type eq 2}">出境线路</c:if> </td> --%>
