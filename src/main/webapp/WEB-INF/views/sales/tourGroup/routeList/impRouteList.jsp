...
<body>
	...
	<div style="margin-top: 30px; margin-left: 18px">
		...
		<%-- <c:if test="${stateFinance==1}">
			<button
				class="button button-primary button-small"
				onclick="importRoute(${tourGroup.id})" disabled="disabled">导入行程</button>
		</c:if> --%>
	</div>
	<div class="p_container">
		<div class="p_container_sub">
			<input type="hidden" id="productId" value="${productInfo.id }"/>
			<input type="hidden" id="stateFinance" value="${stateFinance}"/>
			<input type="hidden" id="state" name="" value="${state}" />
			<form id="routeForm" method="post">
				<input type="hidden" name="tourGroup.id" id="groupId"
					value="${tourGroup.id}" /> <input type="hidden" name="orderId"
				...
				<div>
					...
					<div class="dd_right">
						<input type="text" name="tourGroup.productBrandName"
							value="${productInfo.brandName }" class="IptText100"/>~<input class="IptText300" type="text"
							name="tourGroup.productName"
				...
				<div class="content1">
					<div id="divTab" class="position_relative">
						...
						<dd>
							...
							<textarea class="w_textarea" name="tourGroup.serviceStandard"  class="control-row4 input-large">${productRemark.itemInclude }${productRemark.itemExclude }</textarea>
						</dd>
						<dd>
							<div class="dd_left" style="margin-left: 125px;">备注信息</div>
							<textarea class="w_textarea" name="tourGroup.remark" class="control-row4 input-large">${productRemark.childPlan }${productRemark.shoppingPlan }${productRemark.itemCharge }${productRemark.itemFree }</textarea>
						</dd>
						<dd>
							<div class="dd_left" style="margin-left: 125px;">温馨提示</div>
							<textarea class="w_textarea" name="tourGroup.warmNotice" class="control-row4 input-large">${productRemark.attention }${productRemark.itemOther }${productRemark.warmTip }</textarea>
						</dd>
	...
	<script type="text/javascript"
		src="<%=ctx%>/assets/js/web-js/sales/impRouteList.js"></script>
	<script type="text/javascript"
