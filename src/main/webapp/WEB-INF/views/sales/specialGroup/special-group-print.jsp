...
<!--startprint1-->
	...
	<h6 style="font-size: 30px;" align="center">一地散订单</h6> </div>
	...
	<tbody>
		<c:forEach items="${page.result}" var="groupOrder" varStatus="v">
              ...
              <td style="text-align: left;font-weight: bold;">${groupOrder.supplierName}-${groupOrder.contactName} </td>
              <td style="text-align: left;font-weight: bold;">${groupOrder.receiveMode}</td>
              <td style="font-weight: bold;">${groupOrder.provinceName}${groupOrder.cityName}</td>
              <td style="font-weight: bold;">${groupOrder.numAdult }+${groupOrder.numChild}+${groupOrder.numGuide} </td>
              <td style="font-weight: bold;"><fmt:formatNumber value="${groupOrder.total}" type="currency" pattern="#.##" /></td>
              <td style="font-weight: bold;"><c:if test="${groupOrder.orderLockState==0 }">未锁单</c:if> <c:if test="${groupOrder.orderLockState==1 }">已锁单</c:if></td>
		...
		<c:set var="pageTotalGuide" value="${pageTotalChild+groupOrder.numGuide }"/>
		<c:set var="pageTotal" value="${pageTotal+groupOrder.total }"/>
		</c:forEach>
		
		<tr>
			<td colspan="7" style="font-weight: bold;" >合计:</td>
			<td style="font-weight: bold;">${pageTotalAdult}大${pageTotalChild}小${ pageTotalGuide}陪</td>
...
<table style="width: 100%;">
			<tr>
				<td style="font-weight: bold;">打印人：${printName}&nbsp;&nbsp; 打印时间：${printTime}
						</td>
			</tr>
		</table>
<!--endprint1-->
