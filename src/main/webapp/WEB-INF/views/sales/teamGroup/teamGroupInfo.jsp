...
<script type="text/javascript"
	src="<%=ctx%>/assets/js/web-js/sales/teamGroup.js"></script>
<script type="text/javascript"
...
<script type="text/javascript">
	...
	function toRequirement(){
		if($("input[name='groupOrder.id']").val().trim()==""){
			...
			window.location = "../teamGroup/toRequirement.htm?orderId=${teamGroupVO.groupOrder.id}&operType=${operType}";
		}
	}
	...
	function toTeamGroupInfo(){
		if($("input[name='groupOrder.id']").val().trim()==""){
			...
			window.location = "../teamGroup/toEditTeamGroupInfo.htm?groupId=${teamGroupVO.tourGroup.id}&operType=${operType}";
		}
	}
</script>
				...
				<table border="0" cellspacing="0" cellpadding="0" class="searchTab">
					...
					<tr>
						...
						<td>
							...
							<input type="hidden" name="groupOrder.supplierName" id="supplierName" value="${teamGroupVO.groupOrder.supplierName }" />
							<%-- <input name="groupOrder.supplierName" id="supplierName" type="text" class="IptText300" value="${teamGroupVO.groupOrder.supplierName }" /> --%>
							<input id="supplierName_t" type="text" class="IptText300" value="${teamGroupVO.groupOrder.supplierName }" />
							<c:if test="${operType==1}"><a href="javascript:void(0)" onclick="selectSupplier();">请选择</a></c:if>
						</td>
						<td>组团社团号：</td>
						<td>
							<input type="text" name="groupOrder.supplierCode" class="IptText300" placeholder="组团社团号" value="${teamGroupVO.groupOrder.supplierCode }" />
						</td>
					</tr>
					<tr>
						<td>组团社联系人：</td>
									...
									<c:forEach items="${teamGroupVO.groupOrderPriceList }" var="price"
										...
										<tr>
											...
											<td><input style="width: 80%;<c:if test="${price.stateFinance==1 }">border-color: red </c:if>" <c:if test="${price.stateFinance==1 }">readonly="readonly" </c:if> type="text"
												name="groupOrderPriceList[${index.index}].unitPrice" data-rule-required="true" data-rule-number="true"
											...
											<td><input style="width: 80%; <c:if test="${price.stateFinance==1 }">border-color: red </c:if>" type="text"
												name="groupOrderPriceList[${index.index}].numPerson" data-rule-required="true" data-rule-number="true"
											...
											<td><input style="width: 80%; <c:if test="${price.stateFinance==1 }">border-color: red </c:if>" type="text"
												name="groupOrderPriceList[${index.index}].numTimes" data-rule-required="true" data-rule-number="true"
									...
									<c:forEach items="${teamGroupVO.groupOrderCostList }" var="cost"
										...
										<tr>
											...
											<td><input style="width: 80%" type="text"
												name="groupOrderCostList[${index.index}].unitPrice" data-rule-required="true" data-rule-number="true"
												placeholder="单价" onblur="countTotalCost(${index.index})"
												value="${cost.unitPrice }" /></td>
											<td><input style="width: 80%" type="text"
												name="groupOrderCostList[${index.index}].numPerson" data-rule-required="true" data-rule-number="true"
												placeholder="人数" onblur="countTotalCost(${index.index})"
												value="${cost.numPerson }" /></td>
											<td><input style="width: 80%" type="text"
												name="groupOrderCostList[${index.index}].numTimes" data-rule-required="true" data-rule-number="true"
