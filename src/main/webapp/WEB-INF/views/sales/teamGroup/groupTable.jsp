...
<table class="w_table" style="margin-left: 0px">
	<thead>
		<tr>
			...
			<th style="width:7%">发团日期<i class="w_table_split"></i></th>
	...
	<tbody>
       	<c:forEach items="${groupList}" var="gl" varStatus="v">
       		<tr title="创建时间:${gl.tourGroup.createTimeStr},最后修改人:${gl.tourGroup.updateName },最后修改时间:${gl.tourGroup.updateTimeStr}" style="color:<c:if test="${gl.tourGroup.groupState eq 4}">#ee33ee</c:if><c:if test="${gl.stateFinance eq 1 and gl.tourGroup.groupState ne 4}">blue</c:if>" >
              <td>${v.count}</td>
              ...
              <td>
              		<c:if test="${gl.tourGroup.groupState==0}"><span class="log_action update">未确认</span></c:if>
	                <c:if test="${gl.tourGroup.groupState==1}"><span class="log_action normal">已确认 <c:if test="${gl.stateFinance==1}">(审)</c:if></span></c:if>
					<c:if test="${gl.tourGroup.groupState==2}"><span class="log_action delete">已废弃</span></c:if>
					<c:if test="${gl.tourGroup.groupState==3}"><span class="log_action insert">已审核</span></c:if>
              	...
              	<div class="tab-operate">
					...
					<div class="btn-hide" id="asd">
						...
						<c:if test="${gl.tourGroup.groupState!=3 and gl.tourGroup.groupState!=4 and optMap['EDIT']}">
							<c:if test="${gl.orderLockState != 1 and gl.stateFinance != 1}">
