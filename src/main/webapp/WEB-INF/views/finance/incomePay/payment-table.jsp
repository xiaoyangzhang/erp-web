	...
	<tbody>
		<c:forEach items="${pageBean.result}" var="record" varStatus="status">
			<tr>
				...
				<td>
					<a class="def" onclick="newWindow('付款详情', '<%=staticPath%>/financePay/paymentView?payId=${record.id }')" href="javascript:void(0);">查看</a>
					<a class="def" onclick="toPaymentEdit('${record.id }')" href="javascript:void(0);">修改</a>
					<c:if test="${empty record.detailCount}">
						<a class="def" onclick="deletePayment('${record.id }')" href="javascript:void(0);">删除</a>
					</c:if>
				</td>
