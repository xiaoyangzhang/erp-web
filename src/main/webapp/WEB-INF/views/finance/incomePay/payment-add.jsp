	...
	<div class="p_container_sub">
		<dl class="p_paragraph_content">
			<form method="post" id="payForm">
				...
				<p class="p_paragraph_title"><b>付款信息</b></p>
			...
			<div>
				...
				<div  style="width: 50%;">
					<button type="button" onclick="paymentOrderAdd()" class="button button-primary button-small mt-10">添加</button>
				</div>
			</div>
...
<script type="text/javascript">
						...
						success : function(data) {
							if (data.sucess == true) {
	   			            	$.success("保存成功",function(){
				            		refreshWindow("修改付款处理","../financePay/paymentAdd.htm?payId="+data['supplierId']);
	   			            	});
	   			            }else{
	   							$.error("操作失败");
	   						}
	...
	function paymentOrderAdd(){
		...
		layer.open({
			...
			area: ['1000px', '540px'],
			shadeClose: false,
			content: "paymentOrderPayAddList.htm?supplierId=" + supplierId + "&supType=" + supType,
			btn: ['确定', '取消'],
	...
	function payDetailInsert(supType,payId,details){
		...
		$.ajax({
			type:"post",
			url:"../financePay/paymentDetailInsert.do",
			...
			success:function(data){
				if (data.success == true) {
	            	$.success("保存成功",function(){
	            	//2、插入成功后将在查询出信息进行显示
            		//refreshWindow("修改付款处理","../financePay/loadOrderPayTable.do?ids="+details+"&supplierType="+supType+"&payId="+payId);
	...
	function loadJoinedTableExist() {
		...
		data.rp = "finance/incomePay/payment-list-table";
		$("#tableDivExist").load("../common/queryList.htm", data);
	}
	
	
	
	//点击单条订单记录，移除时删除付款明细子表
