...
<table cellspacing="0" cellpadding="0" class="w_table">
	...
	<tbody>
		<c:forEach items="${pageBean.result}" var="item" varStatus="status">
			...
			>
				<td>
					<input type="checkbox" <c:if test="${item.group_state ne 3 }">disabled</c:if> value="${item.id }"/>
				</td>
				...
				<td>
					...
					<c:if test="${empty item.total_income}">
						0
					</c:if>
				</td>
				<td>
					...
					<c:if test="${empty item.total_cost}">
						0
					</c:if>
				</td>
				<td>
					...
					<c:if test="${empty item.total_profit}">
						0
					</c:if>
				</td>
				<td>
					...
					<c:if test="${item.group_state eq 4}">
						<a class="def" onclick="unseal(${item.id })" href="javascript:void(0);">解封</a>
					</c:if>
				</td>
			</tr>
