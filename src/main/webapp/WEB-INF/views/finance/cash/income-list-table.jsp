	...
	<tbody>
		<c:forEach items="${pageBean.result}" var="record" varStatus="status">
			<tr>
				...
				<td style="text-align: left;">${record.pay_code}</td>
				<td><fmt:formatDate value="${record.pay_date}" pattern="yyyy-MM-dd" /></td>
				<td>${record.left_bank}</td>
...
<%-- 				<td>${record.right_bank_holder}</td> --%>
				...
				<td>
					<a class="def" onclick="newWindow('收款详情', '<%=staticPath%>/finance/incomeView.htm?payId=${record.id }')" href="javascript:void(0);">查看</a>
