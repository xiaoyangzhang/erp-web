	...
	<script type="text/javascript">
		...
		function orderJoin() {
			...
			layer.open({
				...
				area: ['1000px', '480px'],
				shadeClose: false,
				content: "incomeJoinList.htm?supplierId=" + supplierId + "&supType=" + supType,//
				btn: ['确定', '取消'],
...
// 			removeRepeat();
		...
		function pay() {
			$("form").validate({
				...
				submitHandler: function (form) {
					...
					if (confirm("确认保存吗？")) {
						$("#btnSave").attr("disabled", "disabled").attr("value", "保存.."); //防止重复提交
						$("#details").val(JSON.stringify(arr));
						YM.post("pay.do", YM.getFormData("payForm"), function (data) {
							location.href = "incomeView.htm?payId=" + data;
						});
					}
		...
		//选择供应商以后加载账户信息
		function getSupplierBankAccountById(sid, isInit) {
			$.ajax({
				type: 'get',
		...
		<div class="p_container_sub">
			<dl class="p_paragraph_content">
				...
				<div class="payment_footer">
					...
					<p class="">
						——— 本次共计收款：<label id="amountSum"></label>元 ———
					</p>
					<a href="javascript:void(0)" class="btn_gldd" onclick="orderJoin()"><b>关联订单</b></a><br/>
					<button href="#" onclick="pay()" id="btnSave" class="button button-primary button-small mt-10">保存
					</button>
