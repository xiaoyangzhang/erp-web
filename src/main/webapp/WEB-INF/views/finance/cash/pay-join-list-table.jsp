		...
		<c:forEach items="${pageBean.result}" var="order" varStatus="status">
			<tr>
				<td><input type="checkbox" sid="${order.id}" groupCode="${order.group_code}"
				           ...
				           <c:if test="${order.state_finance ne 1}">disabled</c:if>
				/></td>
				...
				<td>${order.total_adult}大${order.total_child}小</td>
...
<div style="overflow-y:scroll;">
	<table cellspacing="0" cellpadding="0" class="w_table">
		...
		<tr>
			...
			<td colspan="2" style="text-align:right;">选中合计：</td>
			...
			if (isNaN(amounts)) {
				...
				if (amounts > 0) {
					tbObj.each(function () { // 循环计算
						...
						if (!checkboxObj.attr("disabled")) { // 状态为已审核
							if (amounts > 0) {
								if (itemBalance > 0) { // 正数
