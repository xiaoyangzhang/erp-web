...
function queryList(page,pagesize) {	
    ...
    var options = {
   		url:"<%=path%>/finance/querySettleCommission.do",
    	type:"post",
    	dataType:"html",
...
function showRecord(o) {
	...
	data.payDirect = 0;
	data.sl = "fin.selectCommCashRecordList";
...
function commPay(index){
	$("#payForm").validate({
		...
		submitHandler : function(form) {
			YM.post("<%=path%>/finance/guide/payCommBill.do",YM.getFormData("payForm"),function(){
				$.success("付款成功");
				queryList();
				clearLayerPayMsg();
				layer.close(index);
