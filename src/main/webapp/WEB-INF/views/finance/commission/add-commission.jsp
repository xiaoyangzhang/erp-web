			...
			<table id="commissionTable" cellspacing="0" cellpadding="0" class="w_table">
				...
				<c:set var="aa" value="0"></c:set>
					...
					<tr>
						...
						<td ${disabledCss }>
							<input type="radio" name="cashType${status.index}" value="发放" ${inputDisabled } checked="checked" >发放</input>
						</td>
						<td ${disabledCss }>
							<c:set var="total" value="${com.total}" />
...
function getRowHtml(index){
	...
	arr.push('<tr>');
		...
		arr.push('</td>');
		arr.push('<td>');
			arr.push('<input type="radio"  name="cashType'+index+'" value="发放" checked="checked">发放</input>');
		arr.push('</td>');
