    ...
    <form id="queryForm">
	    ...
	    <div class="p_container_sub">
	    	<dl class="p_paragraph_content">
	    			...
	    			<div class="dd_right grey">	
	    				...
	    				<select id="sjType" name="sjType">
	    					<c:forEach items="${sup_type_map}" var="map">
	    						<c:if test="${map.value eq '酒店' }">
	    							<option value="${map.key }">${map.value}</option>
	    						</c:if>
	    					</c:forEach>
	    					<c:forEach items="${sup_type_map}" var="map">
	    						<c:if test="${map.value ne '酒店' }">
	    							<option value="${map.key }">${map.value}</option>
	    						</c:if>
	    					</c:forEach>
	    				</select>
	    		...
	    		<input type="hidden" id="sjType" name="sjType" value="120"/>
	    		<dd class="inl-bl pl-10">
	    			<div class="dd_right grey">	
	    				商家名称:					
...
function queryList(page,pagesize) {	
    ...
    var options = {
   		url:"<%=path%>/finance/subjectSummary3.do",
    	type:"post",
    	dataType:"html",
...
function print(){
	window.open ("<%=staticPath%>/finance/sumjectSummaryPrint3.htm?"+ $("#queryForm").serialize());
}
