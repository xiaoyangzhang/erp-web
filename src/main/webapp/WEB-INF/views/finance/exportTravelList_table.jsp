...
<table cellspacing="0" cellpadding="0" class="w_table">
	...
	<thead>
		<tr>
			...
			<th>导出状态<i class="w_table_split"></i></th>
			<th>收入<i class="w_table_split"></i></th>
			<th><i class="w_table_split"></i>
				<input type="checkbox" id="checkAllExportTravelStatus" onclick="checkAllExportGroup(this)"/>
			</th>
		</tr>
	</thead>
	<tbody>
		<c:forEach items="${pageBean.result}" var="item" varStatus="status">
			...
			>
				...
				<td <c:if test="${item.travel_export_status==0}">style="color:red"</c:if>><c:if test="${item.travel_export_status==0}">未导出</c:if><c:if test="${item.travel_export_status==1}">已导出</c:if></td>
				<td>
				...
				<td>
					<input type="checkbox" value="${item.id}" onclick="checkExportGroup(this);" class="check-export-travel-status"/>
				</td>
			</tr>
