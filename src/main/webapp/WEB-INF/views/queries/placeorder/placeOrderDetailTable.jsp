...
<col width="5%" /><col width="10%" /><col width="13%" /><col width="5%" /><col width="4%" /><col width="10%" />
	...
	<thead>
		<tr>
			...
			<th>销售金额(毛利)<i class="w_table_split"></i></th>
		</tr>
	...
	<tbody><input type="hidden"  id="searchPageSize" value="${pageBean.pageSize }"/>
		<c:forEach items="${pageBean.result}" var="v" varStatus="vs">
			...
			<tr>
				...
				<td>${v.productBrandName }-${v.productName }</td> <!-- 产品 -->
				<td>${v.totalAdult }大${v.totalChild }小${v.totalGuide }陪</td>  <!-- 人数 -->
				...
				<td style="text-align: left">${v.supplierName}</td><!-- 酒店 -->
				...
				<!-- 销售价 -->
				<td>	<fmt:formatNumber value="${v.sale_item_price }" pattern="#.##" type="currency"/></td>
				...
				<!-- 销售金额(毛利) -->
				<td ><fmt:formatNumber value="${v.sale_item_price_gp}" pattern="#.##" type="currency"/></td>
