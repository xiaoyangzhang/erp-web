...
<link href="<%=staticPath%>/assets/css/supplier/supplier.css"
	rel="stylesheet" />
<script type="text/javascript"
	src="<%=staticPath%>/assets/js/chart/highcharts.js"></script>
<script type="text/javascript">
	$(function() {
		var vars={
			...
			<div class="p_container_sub">
				<div class="searchRow">
					<ul>
						...
						</select> <input name="startDate" type="text" class="Wdate" id="startDate"
							value="${startDate }"
							onClick="WdatePicker({dateFmt:'yyyy-MM-dd'})" /> ~<input
							name="endDate" id="endDate" type="text" class="Wdate" value="${endDate }"
							onClick="WdatePicker({dateFmt:'yyyy-MM-dd'})" /></li>
						<li class="text">组团社名称</li>
						<li><input name="supplierName" type="text" id="supplierName"/></li>
						<li class="text">产品</li>
						<li><input id="productName" name="productName" type="text"
							placeholder="输入产品名称或品牌" /></li>
						<li class="clear" />
						<li class="text"></li>
						...
						<li class="clear" />
						<li><input type="button" id="btnQuery" onclick="queryList()"
							class="button button-primary button-small" value="查询">
							<a href="javascript:void(0)" id="preview6" onclick="toPreview6()"  class="button button-glow button-rounded button-raised button-primary button-small" >
						打印预览</a>
							</li>
					</ul>
				</div>
...
<script type="text/javascript">
		...
		layer.open({
			...
			yes : function(index) {
				...
				for (var i = 0; i < userArr.length; i++) {
					console.log("id:" + userArr[i].id + ",name:"
							+ userArr[i].name + ",pos:" + userArr[i].pos
							+ ",mobile:" + userArr[i].mobile + ",phone:"
							+ userArr[i].phone + ",fax:" + userArr[i].fax);
					if (i == userArr.length - 1) {
						$("#operatorName").val(
								$("#operatorName").val() + userArr[i].name);
						$("#operatorIds").val(
								$("#operatorIds").val() + userArr[i].id);
					} else {
						$("#operatorName").val(
								$("#operatorName").val() + userArr[i].name
										+ ",");
	...
	function toPreview6(){
		$("#preview6").attr("target","_blank").attr("href","../query/productGuestDetailPreview.htm?dateType="+$("#dateType").val()+"&startDate="+
