...
<table cellspacing="0" cellpadding="0" class="w_table" id="payTable">
	...
	<tbody>
		<c:forEach items="${pageBean.result}" var="v" varStatus="vs">
			<tr>
				...
				<td>
					<button type="button" class="button button-rounded button-tinier" onclick="toEntertainmentDetail(${v.supplierId})">查看明细</button>
				</td>
			</tr>
	...
	<tbody>
		<tr>
			<td></td>
			<td colspan="1" style="text-align: left">合计：</td>
		    <td >${sum_orderCount}</td>
		    
		    <td ><fmt:formatNumber value="${sum_roomCount}" pattern="#.##" type="number"/></td>
