	...
	<tbody>
		<c:forEach items="${pageBean.result}" var="item" varStatus="status">
			<tr>
				...
				<td ><fmt:formatNumber value="${item.totalSale}" pattern="#.##"/></td>
				<td ><fmt:formatNumber value="${item.total-item.totalSale}" pattern="#.##"/></td>
		...
		<tr>
			...
			<td ><fmt:formatNumber value="${sum_sale }" pattern="#.##"/></td>
			<td ><fmt:formatNumber value="${sum_total-sum_sale }" pattern="#.##"/></td>
			<td></td>
		</tr>
...
<script type="text/javascript">
	...
	function exportExcel(){
		location.href="saleDeliveryExportExcel.do?"+(window.location.search.length>0?location.search.substring(1):"");
	}
