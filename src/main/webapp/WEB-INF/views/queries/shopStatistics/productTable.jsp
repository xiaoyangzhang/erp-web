...
<table cellspacing="0" cellpadding="0" class="w_table" id="payTable">
	...
	<thead>
		<tr>
			...
			<th>差额/完成比<i class="w_table_split"></i></th>
	...
	<tbody>
		<c:forEach items="${pageBean.result}" var="v" varStatus="vs">
			<tr>
				<td>${vs.count}</td>
				<td>【${v.product_brand_name }】${v.product_name }</td>
				<td><fmt:formatNumber value="${v.group_num }" type="currency" pattern="#.##"/></td>
				<td>
					<fmt:formatNumber value="${v.total_adult}" type="currency" pattern="#.##"/>大
					<fmt:formatNumber value="${v.total_child}" type="currency" pattern="#.##"/>小
				</td>
				<td><fmt:formatNumber value="${v.shop_count }" type="currency" pattern="#.##"/></td>
				...
				<td><fmt:formatNumber value="${v.factSales-v.planSales }" type="currency" pattern="#.##"/>/
				...
				<td>
				<c:choose>
			<%-- <c:when test="${v.person_num ne 0 and v.factSales  ne 0.0000 }">
			<fmt:formatNumber type="number"  value="${v.factSales /v.person_num}" pattern="#.##" /></c:when> --%>
			...
			<c:otherwise><fmt:formatNumber type="number" value="0" pattern="#.##" /></c:otherwise>
				...
				<%-- <c:set var="numPersonGroup" value="${numPersonGroup+v.total_person}" /> --%>
				<c:set var="count_total_adult" value="${count_total_adult+v.total_adult }"></c:set>
				<c:set var="count_total_child" value="${count_total_child+v.total_child }"></c:set>
		...
		<tr>
			...
			<td>
				<fmt:formatNumber value="${count_total_adult}" type="currency" pattern="#.##"/>大
				<fmt:formatNumber value="${count_total_child}" type="currency" pattern="#.##"/>小
			</td>
