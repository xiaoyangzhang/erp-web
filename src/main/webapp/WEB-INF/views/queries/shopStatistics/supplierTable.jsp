	...
	<tbody>
		<c:forEach items="${pageBean.result}" var="v" varStatus="vs">
			<tr>
				...
				<td><fmt:formatNumber value="${v.countGroup }" type="currency" pattern="#.##"/></td>
				<td><fmt:formatNumber value="${v.numPersonGroup }" type="currency" pattern="#.##"/></td>
				<td><fmt:formatNumber value="${v.countShop }" type="currency" pattern="#.##"/></td>
				...
				<c:set var="countGroup" value="${countGroup+v.countGroup}" />
				<c:set var="numPersonGroup" value="${numPersonGroup+v.numPersonGroup }" />
				<c:set var="countShop" value="${countShop+v.countShop }" />
				...
				<c:if test="${v.planSales!='0.0000' and v.factSales!='0.0000' }">
						<c:set var="fvp" value="${fvp+v.factSales/v.planSales }" />
				</c:if>
		...
		<tr>
			...
			<td><fmt:formatNumber value="${fvp }" type="percent" pattern=""/></td>
