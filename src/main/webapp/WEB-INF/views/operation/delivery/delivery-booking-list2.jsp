		...
		<c:forEach items="${list}" var="info" varStatus="status">
			<tr>
				...
				<td>
					...
					<a class="def" href="javascript:void(0)" onclick="newWindow('查看下接社订单','<%=staticPath %>/booking/bookingDeliveryView.htm?gid=${info.groupId }&bid=${info.id}&isShow=${isShow }')">查看</a>
					...
					<c:if test="${optMap['EDIT'] && (booking.stateFinance==null || booking.stateFinance==0 )}">
						<a class="def" href="javascript:void(0)" onclick="newWindow('修改下接社订单','<%=staticPath %>/booking/bookingDeliveryEdit.htm?gid=${info.groupId }&bid=${info.id}&isShow=${isShow }')">修改</a>
					</c:if>		
