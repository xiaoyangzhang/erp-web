	...
	<p class="p_paragraph_title" style="margin: 0;">
		<b>地接社安排</b>
			<a class="button button-primary button-small"
				...
				onclick="newWindow('新增地接订单','<%=ctx %>/booking/newToAddSight?groupId=${groupId}&supplierType=${supplierType}')">新增</a>&nbsp;&nbsp;&nbsp;
		<a class="button button-primary button-small"
					...
					<tr>
						...
						<td>
							<div class="tab-operate">
								...
								onclick="newWindow('查看地接订单','<%=staticPath %>/booking/viewSupplier.do?groupId=${booking.groupId }&bookingId=${booking.id}&supplierId=${booking.supplierId }')">查看</a>
							<a class="def" href="javascript:void(0)"
								onclick="printOrder(${booking.id})">打印</a> <c:if
									test="${ (booking.stateFinance==null || booking.stateFinance==0 ) }">
									<a class="def" href="javascript:void(0)"
										onclick="newWindow('修改地接订单','<%=staticPath %>/booking/newToAddSight?groupId=${booking.groupId }&bookingId=${booking.id}&supplierId=${booking.supplierId }&supplierType=${supplierType}')">修改</a>
								</c:if> <c:if test="${ booking.canDelete==true}">
									<a class="def" href="javascript:void(0)"
