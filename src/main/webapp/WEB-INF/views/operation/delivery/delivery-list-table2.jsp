             ...
             <tr> 
	              ...
	              <c:choose>
                  		<c:when test="${groupInfo.groupMode < 1}">
                  			<a class="def" href="javascript:void(0)" onclick="newWindow('查看散客团信息','<%=ctx %>/fitGroup/toFitGroupInfo.htm?groupId=${groupInfo.groupId}&operType=0&isShow=${isShow }')">${groupInfo.groupCode}</a></td>
                  		</c:when>
                  		<c:otherwise>
				 			<a class="def" href="javascript:void(0)" onclick="newWindow('查看定制团信息','<%=ctx %>/teamGroup/toEditTeamGroupInfo.htm?groupId=${groupInfo.groupId }&operType=0&isShow=${isShow }')">${groupInfo.groupCode}</a></td> 
                  		</c:otherwise>
                  	</c:choose>
	              </td> 
                  ...
                  <c:if test="${optMap['EDIT'] }">
	                  <td>
	                  	<c:if test="${groupInfo.groupState eq 1 }">
	               			<a class="def"  href="javascript:void(0)" onclick="newWindow('新增下接社','<%=staticPath %>/booking/bookingDeliveryEdit.htm?gid=${groupInfo.groupId }&isShow=${isShow }')">新增</a>
	                  	</c:if>
	                  </td>
                  </c:if>                  
             </tr>
	...
	$(function(){
		...
		var divExpand = function (btnObj, groupID) {
		 ...
		 function loadData(containerId,gid){
			 var isShow = "${isShow}";
			 $("#"+containerId).load("deliveryBookingList2.htm?gid="+gid+"&isShow="+isShow);
		 }
	})
