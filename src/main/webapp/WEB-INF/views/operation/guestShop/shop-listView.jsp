...
<table cellspacing="0" cellpadding="0" class="w_table" >
	<col width="4%" /><col width="10%" /><col width="6%" /><col width="4%" /><col width="20%" />
	<col width="15%" /><col width="6%" /><col width="5%" /><col width="4%" /><col width="5%" />
	<col width="10%" />
	<thead>
	<tr>
	...
	<c:forEach items="${page.result}" var="groupInfo" varStatus="status">
		<tr>
			...
			<td style="text-align: left;"><c:if test="${groupInfo.bookSupplierName!=null }">
				                  ${fn:replace(groupInfo.bookSupplierName,',','</br>') }
				                  	 
				                  	</c:if></td>
			<td>
					<c:if test="${groupInfo.groupState ne 0 }">
					<%-- <c:if test="${optMap['SHOP'] }"> --%>
					<!-- 	<a class="def" href="">分配购物店</a> -->
						<a class="def" href="javascript:void(0)" onclick="newWindow('分配购物店','<%=staticPath %>/bookingShop/toFinanceBookingShopView.htm?groupId=${groupInfo.groupId }&type=1')">分配购物店</a>
					<%-- </c:if> --%>
					<%-- <c:if test="${optMap['TASK'] }"> 
						<!-- <a class="def" href="">分配指标</a> 
						<a class="def" href="javascript:void(0)" onclick="newWindow('分配指标','<%=staticPath %>/bookingShop/toBookingShopView.htm?groupId=${groupInfo.groupId }&type=0')">分配指标</a>
					 </c:if> --%>
				</c:if>
<%-- 				<a class="def" href="javascript:void(0)" onclick="newWindow('新增购物','<%=staticPath %>/bookingFinanceShop/toAddShop.htm?groupId=${groupInfo.groupId }&type=1')">新增</a>
 --%>			<c:if test="${groupInfo.bookSupplierName!=null and groupInfo.groupState ne 0  }">
				<a class="def" href="javascript:void(0)" onclick="newWindow('修改购物','<%=staticPath %>/bookingGuestShop/toEditShop.htm?groupId=${groupInfo.groupId }&type=0')">编辑</a>
			</c:if>
			</td> 
		</tr>
	</c:forEach>
