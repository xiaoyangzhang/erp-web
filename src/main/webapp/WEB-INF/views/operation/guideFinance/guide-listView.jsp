...
<%-- 				                  		<div class="serialnum_btn" groupId="${groupInfo.groupId}"></div> --%>
				                 ...
				                 <%--  <c:if test="${!fn:contains(groupInfo.groupStatus, '未确认')}">
		          					...
		          					<a class="def" href="javascript:void(0)" onclick="newWindow('报账单','<%=staticPath %>/bookingGuideFinance/finance.htm?bookingId=${groupInfo.bookingGuideId}&groupId=${groupInfo.groupId}')">报账单</a>
		          					 <c:if test="${groupInfo.groupMode==0}">
				                  		<a class="def" href="download.htm?guideId=${groupInfo.bookingGuideId}&num=2">打印出团单</a>
				                  	 </c:if>
				                  	 <c:if test="${groupInfo.groupMode>0}">
				                  		<a class="def" href="download.htm?guideId=${groupInfo.bookingGuideId}&num=1">打印出团单</a>
				                  	 </c:if>
				                  </td>
				               </tr>
			              </c:forEach>
