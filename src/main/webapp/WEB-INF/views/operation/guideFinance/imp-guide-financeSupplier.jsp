	    					...
	    					<tbody>
	    					 <c:forEach items="${list}" var="list" varStatus="status">
		    						<tr>
			    						...
			    						<td><fmt:formatNumber value="${list.total }" type="number" pattern="0.00"/></td>
			    						<td>
			    						<c:choose>
			          						<c:when test="${list.totalCash eq null  }">0.00</c:when>
			          						<c:otherwise><fmt:formatNumber value="${list.totalCash }" type="number" pattern="0.00"/></c:otherwise>
			          					</c:choose>
			    						</td>
			    						<td><fmt:formatNumber value="${list.total-list.totalCash }" type="number" pattern="0.00"/></td>
			    						<td id="${status.index+1}">
				    						...
				    						<input type="text" name="total" style="width:80%;" value="<fmt:formatNumber value="${list.total-list.totalCash }" type="number" pattern="0.00"/>"/>
			    						</td>
		    						</tr>
	    						</c:forEach>
