	    		...
	    		<dd>
	    			<div class="useroom_tab pl-20 pr-20">
	    				<table cellspacing="0" cellpadding="0" border="1" class="w_table">
	    					<col width="5%"/><col width="10%"/><col width="10%"/><col width="5%"/><col width=""/><col width="5%"/>
	    					...
	    					<tbody>
	    					 <c:forEach items="${list}" var="list" varStatus="status">
		    						<tr id="${list.fid }">
			    						...
			    						<td>
			    							<c:if test="${not empty list.totalCash }">
			    								<fmt:formatNumber value="${list.totalCash }" type="number" pattern="#.##"/>
			    							</c:if>
			    							<c:if test="${empty list.totalCash }">0</c:if>
			    						</td>
			    						...
			    						<td><c:if test="${stateFinance ne 1 }"><a href="#" onclick="delFinance(${list.groupId }, ${list.id },${financeGuide.bookingId})" class="def">删除</a></c:if></td>
<%-- 			    						<td></td> --%>
		    						</tr>
	    				...
	    				<div class="ml-30">
	    					...
	    					<c:if test="${stateFinance ne 1 }">
	    					<c:choose>
							    <c:when test="${financeGuide.supplierType eq 5}">
							    	<a class="button button-primary button-rounded button-small mt-20 mb-20" onclick="imp()">关联</a>
							    </c:when>
							    <c:when test="${financeGuide.supplierType eq 3}">
							    	<a class="button button-primary button-rounded button-small mt-20 mb-20" onclick="imp()">关联</a>
							    </c:when>
							    <c:when test="${financeGuide.supplierType eq 2}">
							    	<a class="button button-primary button-rounded button-small mt-20 mb-20" onclick="imp()">关联</a>
							    </c:when>
							    <c:when test="${financeGuide.supplierType eq 4}">
							    	<a class="button button-primary button-rounded button-small mt-20 mb-20" onclick="imp()">关联</a>
							    </c:when>
							    <c:when test="${financeGuide.supplierType eq 120}">
							    	<a class="button button-primary button-rounded button-small mt-20 mb-20" onclick="imp()">关联</a>
							    </c:when>
							    <c:when test="${financeGuide.supplierType eq 121}">
							    	<a class="button button-primary button-rounded button-small mt-20 mb-20" onclick="imp()">关联</a>
							    </c:when>							   
							</c:choose>
	    					<c:choose>		
							    <c:when test="${financeGuide.supplierType eq 5}">
							    	<a class="button button-primary button-rounded button-small mt-20 mb-20" href="javascript:void(0)"  
							    	onclick="newWindow('新增景点门票','<%=ctx %>/booking/toAddSight?groupId=${financeGuide.groupId}')">新增景点门票</a>
							    </c:when>
							    <c:when test="${financeGuide.supplierType eq 3}">
							    	<a class="button button-primary button-rounded button-small mt-20 mb-20" href="javascript:void(0)"  
							    	onclick="newWindow('新增用房','<%=ctx %>/booking/toAddHotel?groupId=${financeGuide.groupId}')">新增用房</a>
							    </c:when>
							    <c:when test="${financeGuide.supplierType eq 2}">
							    	<a class="button button-primary button-rounded button-small mt-20 mb-20" href="javascript:void(0)"  
							    	onclick="newWindow('新增用餐','<%=ctx %>/booking/toAddEat?groupId=${financeGuide.groupId}')">新增用餐</a>
							    </c:when>
							    <c:when test="${financeGuide.supplierType eq 4}">
								    <a class="button button-primary button-rounded button-small mt-20 mb-20" href="javascript:void(0)"  
								    onclick="newWindow('新增用车','<%=ctx %>/booking/toAddCar?groupId=${financeGuide.groupId}')">新增用车</a>
							    </c:when>
							    <c:when test="${financeGuide.supplierType eq 120}">
								    <a class="button button-primary button-rounded button-small mt-20 mb-20" href="javascript:void(0)"  
								    onclick="newWindow('新增其他收入','<%=ctx %>/booking/editIncome.htm?groupId=${financeGuide.groupId}')">新增其他收入</a>
							   	</c:when>
								<c:when test="${financeGuide.supplierType eq 121}">
									<a class="button button-primary button-rounded button-small mt-20 mb-20" href="javascript:void(0)" 
									onclick="newWindow('新增其他支出','<%=ctx %>/booking/editOutcome.htm?groupId=${financeGuide.groupId}')">新增其他支出</a>
								</c:when>							   
							</c:choose>
							</c:if>
	    				</div>
