            	...
            	<dd>
            		<ul class="con_box">
            		 <c:forEach items="${list}" var="list">
            			<c:if test="${list.supplierType eq 5 }">
            				<a href="financeSupplierView.htm?groupId=${groupId }&bookingId=${bookingId}&supplierType=5">
            					<li>
            						<p>景点门票</p>
            						<p>￥<fmt:formatNumber value="${list.total }" type="number" pattern="0.00"/></p>
            					</li>
            				</a>
            			</c:if>
            			<c:if test="${list.supplierType eq 3 }">
            				<a href="financeSupplierView.htm?groupId=${groupId }&bookingId=${bookingId}&supplierType=3">
            					<li>
            						<p>用房</p>
            						<p>￥<fmt:formatNumber value="${list.total }" type="number" pattern="0.00"/></p>
            					</li>
            			...
            			<c:if test="${list.supplierType eq 2 }">
            				<a href="financeSupplierView.htm?groupId=${groupId }&bookingId=${bookingId}&supplierType=2">
            					<li>
            						<p>用餐</p>
            						<p>￥<fmt:formatNumber value="${list.total }" type="number" pattern="0.00"/></p>
            					</li>
            			...
            			<c:if test="${list.supplierType eq 4 }">
            				<a href="financeSupplierView.htm?groupId=${groupId }&bookingId=${bookingId}&supplierType=4">
            					<li>
            						<p>用车</p>
            						<p>￥<fmt:formatNumber value="${list.total }" type="number" pattern="0.00"/></p>
            					</li>
            			...
            			<c:if test="${list.supplierType eq 120 }">
            				<a href="financeSupplierView.htm?groupId=${groupId }&bookingId=${bookingId}&supplierType=120">
            					<li>
            						<p>其他收入</p>
            						<p>￥<fmt:formatNumber value="${list.total }" type="number" pattern="0.00"/></p>
            					</li>
            			...
            			<c:if test="${list.supplierType eq 121 }">
            				<a href="financeSupplierView.htm?groupId=${groupId }&bookingId=${bookingId}&supplierType=121">
            					<li>
            						<p>其他支出</p>
            						<p>￥<fmt:formatNumber value="${list.total }" type="number" pattern="0.00"/></p>
            					</li>
            		...
            		<div class="btn ml-30 mt-50">
            			...
            			<c:if test="${reqpm.isHidden ne true}">
	            			...
	            			<c:if test="${guidesVo.guide.stateBooking eq 2 && !empty fromfin && fromfin eq 1 }">
		            			...
		            			<c:if test="${guidesVo.guide.stateFinance eq 1 }">已审核</c:if>
	            			</c:if>
	            		</c:if>
	            		<a href="<%=staticPath%>/bookingGuide/printDetail.htm?isHidden=true&isPrint=true&groupId=${groupId }&bookingId=${bookingId}"
	            			class="button button-primary button-rounded button-small mr-20" target="_blank">打印</a>
