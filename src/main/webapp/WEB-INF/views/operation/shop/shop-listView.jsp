...
<col width="4%" /><col width="15%" /><col width="6%" /><col width="4%" />
<col width="16%" /><col width="15%" /><col width="7%" /><col width="8%" /><col width="5%" /><col width="11%" /><col width="10%" />
	<thead>
	<tr>
	...
	<c:forEach items="${page.result}" var="groupInfo" varStatus="status">
		<tr>
			...
			<td style="text-align:left"><c:if test='${groupInfo.bookSupplierName!=null}'>${fn:replace(groupInfo.bookSupplierName, ",", "<br>") }</c:if></td>
			...
			<td>
				<c:if test="${groupInfo.groupState eq 1}">
					...
					<!-- 	<a class="def" href="">分配购物店</a> -->
						<a class="def" href="javascript:void(0)" onclick="newWindow('分配购物店','<%=staticPath %>/bookingShop/toBookingShopView.htm?groupId=${groupInfo.groupId }&type=1')">分配购物店</a>
					</c:if>
				...
				<a class="def" href="javascript:void(0)" onclick="newWindow('查看购物','<%=staticPath %>/bookingShop/bookingShopView.htm?groupId=${groupInfo.groupId }&type=0')">查看</a>
			</td>
		</tr>
	</c:forEach>
	</tbody>
