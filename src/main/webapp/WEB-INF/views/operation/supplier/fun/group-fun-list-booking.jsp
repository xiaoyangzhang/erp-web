...
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
 <%@ include file="../../../../include/path.jsp" %>
          	...
          	<div class="in_tab mb-30">
          		...
          		<p class="in_tab_title"><b>景区订单</b></p>
          		<form action="" id="bookingFun">
          		<table class="in_tab_body" border="1" cellspacing="0" cellpadding="0">
          			<col width="20%" /><col width="10%" /><col width="10%" /><col width="20%" /><col width="10%" />
 					<col width="10%" /><col width="10%" />
          			...
          			<tr>
						...
						<c:otherwise>
							<c:if test="${booking.stateBooking==0 }">娱乐未确认</c:if>
							
							<c:if test="${booking.stateBooking==1 }">娱乐已确认</c:if>
          				...
          				<td>
							<a class="def" href="javascript:void(0)" onclick="newWindow('查看娱乐订单','<%=staticPath %>/booking/viewSupplier.do?groupId=${booking.groupId }&bookingId=${booking.id}')">查看</a>
          				<c:if test="${optMap['EDIT'] && (booking.stateFinance==null || booking.stateFinance==0 ) && booking.stateBooking==0}">
						<a class="mr-10 blue" href="javascript:void(0)" onclick="confirm(this,${booking.id })">确认</a>
						</c:if>
							...
							<c:if test="${optMap['EDIT'] && (booking.stateFinance==null || booking.stateFinance==0 ) }">
		         			<a class="def" href="javascript:void(0)" onclick="newWindow('修改娱乐订单','<%=staticPath %>/booking/toAddFun?groupId=${booking.groupId }&bookingId=${booking.id}')">修改</a>
							</c:if>
...
<script type="text/javascript">
	...
	function confirm(obj,id){
		$.post("stateConfirm.do",{id:id},function(data){
	   		if(data.success){
	   			$(obj).hide();
	   			$(obj).parent().prev(".state").html("娱乐场所已确认");
	   			$.info('确认成功！');	   			
	   		}else{
	   			$.info(data.msg);
	   		}
