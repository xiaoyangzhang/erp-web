...
<body>
					...
					<dd>
						...
						<div class="dd_right">
							...
							<input type="hidden" name="supplierId" id="supplierId"
								...
								value="选择" class="button button-primary button-small" onclick="selectFun()" id="fun"/>
						</div>
						<div class="clear"></div>
					</dd>
					<dd>
							...
							<table cellspacing="0" cellpadding="0" class="w_table"
								...
								<thead>
									<tr>
										...
										<th><a href="javascript:void(0)" id="funBtn"
											...
											<tr>
													<td><input type="hidden" name="detailId" value="${detail.id }"/>
														<select id="type1Id" name="type1Id" class="select-w80"> 
															<c:forEach items="${funTypes}" var="t1" varStatus="vs">
	...
	<div id="addFunItem" style="display: none;">
		<div class="p_container">
				...
				<form action="" id="detailForm">
					<dl class="p_paragraph_content" style="margin-top: 20px">
						...
						<dd>
							<div class="dd_left"></div>
							<div class="dd_right">
								<input type="submit" id="funSubmit"
									class="button button-primary button-small"
									value="确定" /> <input type="button" id="funCancel"
									class="button button-primary button-small"
									value="取消" />
							</div>
							<div class="clear"></div>
...
<script type="text/html" id="funRow">
...
function showAdd(){
	$("#funTblTr").append($("#funRow").html());
}
function bindEvent(){
	$("#funTblTr tr").each(function(){
		//绑定删除事件
		$(this).find("a[name='del']").unbind("click").bind("click",function(){
			var rowindex = $(this).closest("#funTblTr").find("tr").index($(this).closest("tr")[0]);			
		...
		function calcSum(){
			var sum=0;
			//计算总价				
			$("#funTblTr tr").find("input[name='itemTotal']").each(function(){
...
$(function(){
	<c:if test='${! empty bookingDetailList }'>
	 ...
	 $("#groupDetail").load("<%=staticPath%>/booking/groupDetail.htm?gid="+$("#groupId").val()+"&stype=7");
	 if($("#flag").val()==1){
		 $("#btnSave").attr("style","display:none");
		 $("#fun").attr("style","display:none");
		 $("#contact1").attr("style","display:none");
		 $("#funBtn").attr("style","display:none");
						...
						var options={
						        ...
						        success: function(data) {
						            if (data.sucess ) {		
						            	...
						            	refreshWindow("修改娱乐订单","<%=staticPath %>/booking/toAddFun?groupId="+data['groupId']+"&bookingId="+data['bookingId']+"&stateBooking="+data['stateBooking']);
						            	})
						            }else{
										$.error("操作失败"+data.msg);
									}
	...
	function selectFun(){
		layer.openSupplierLayer({
			title : '选择地点',
			content : '<%=ctx%>/component/supplierList.htm?supplierType=7',
	...
	function delRowEvent(obj) {
		var rowindex = $(obj).closest("#funTblTr").find("tr").index(
				$(obj).closest("tr")[0]);
...
<script type="text/javascript">
    ...
    $(function(){
        var param = "";
        JAutocompleteEx("#supplierName", "<%=staticPath %>/tourGroup/getSupplier?supplierType=7", param, "supplierId", customerTicketCallback, 1);
    });
