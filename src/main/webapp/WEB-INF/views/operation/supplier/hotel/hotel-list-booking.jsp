					...
					<td>
						<div class="tab-operate">
							...
							<div class="btn-hide" id="asd">
								<a class="def" href="javascript:void(0)" onclick="newWindow('查看酒店订单','<%=staticPath %>/booking/viewSupplier.do?groupId=${booking.groupId }&bookingId=${booking.id}&isShow=${isShow }')">查看</a>
								...
								<c:if test="${optMap['EDIT'] && (booking.stateFinance==null || booking.stateFinance==0 ) }">
									<a class="def" href="javascript:void(0)" onclick="newWindow('修改酒店订单','<%=staticPath %>/booking/toAddHotel?groupId=${booking.groupId }&bookingId=${booking.id}&isShow=${isShow }')">修改</a>
								</c:if>
								...
								<c:if test="${optMap['JDDD_NUM_EDIT'] && (booking.stateFinance==null || booking.stateFinance==0 ) }">
									<a class="def" href="javascript:void(0)" onclick="newWindow('修改酒店订单数量','<%=staticPath %>/booking/toAddHotel?groupId=${booking.groupId }&bookingId=${booking.id}&editType=0&isShow=${isShow }')">修改数量</a>
								</c:if>
							</div>
						</div>
					</td>
