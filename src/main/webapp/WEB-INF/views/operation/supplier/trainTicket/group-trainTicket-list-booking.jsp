...
<div class="p_container">
	...
	<p class="p_paragraph_title" style="margin: 0;">
		<b>火车票安排</b>
		<c:if test="${optMap['YDAP_TRAIN'] and groupCanEdit }">
			<a class="button button-primary button-small"
				href="javascript:void(0)"
				onclick="newWindow('新增火车票订单','<%=ctx %>/booking/toAddTrainTicket?groupId=${groupId }')">新增</a>&nbsp;&nbsp;&nbsp;</c:if>
		<a class="button button-primary button-small"
					...
					<tr>
						...
						<td>
							<div class="tab-operate">
								...
								onclick="newWindow('查看火车票订单','<%=staticPath %>/booking/viewSupplier.do?groupId=${booking.groupId }&bookingId=${booking.id}')">查看</a>
								<c:if
									test="${optMap['YDAP_TRAIN'] && (booking.stateFinance==null || booking.stateFinance==0 ) && booking.stateBooking==0}">
									<a class="def" href="javascript:void(0)"
										onclick="confirm(this,${booking.id })">确认</a>
								</c:if> <a class="def" href="javascript:void(0)"
								onclick="printOrder(${booking.id})">打印</a> <c:if
									test="${optMap['YDAP_TRAIN'] && (booking.stateFinance==null || booking.stateFinance==0 ) }">
	
									<a class="def" href="javascript:void(0)"
										onclick="newWindow('修改火车票订单','<%=staticPath %>/booking/toAddTrainTicket?groupId=${booking.groupId }&bookingId=${booking.id}')">修改</a>
								</c:if> <c:if test="${optMap['YDAP_TRAIN'] && booking.canDelete==true}">
									<a class="def" href="javascript:void(0)"
...
<script type="text/javascript">
	function del(obj,id,type){
		...
		$("#bookingTrain").ajaxSubmit(options);
	...
	function confirm(obj,id){
		$.post("stateConfirm.do",{id:id},function(data){
	   		if(data.success){
	   			$(obj).hide();
	   			$(obj).parent().prev(".state").html("火车票已确认");
	   			$.infoR('确认成功！');	   			
	   		}else{
	   			$.infoR(data.msg);
	   		}
