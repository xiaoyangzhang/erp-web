					...
					<dd>
						...
						<div class="dd_right">
							...
							<input type="hidden" name="supplierId" id="supplierId"
								...
								value="选择" onclick="selectTrain()" id="train" />
						</div>
						<div class="clear"></div>
					</dd>
					<dd>
							...
							<table cellspacing="0" cellpadding="0" class="w_table" id="priceTable">
								...
								<thead>
									<tr>
										...
										<th><a href="javascript:void(0)" id="trainBtn"
											...
											<tr>
												<td><input type="hidden" name="detailId" value="${detail.id }"/>
													<select id="type1Id" name="type1Id" class="select-w80"> 
														<c:forEach items="${trainTypes}" var="t1" varStatus="vs">
			...
			<div class="p_container_sub">
				<form action="" id="detailForm">
					<dl class="p_paragraph_content" style="margin-top: 20px">
						...
						<dd>
							<div class="dd_left">始发地:</div>
							<input type="text" name="ticketDeparture" id="ticketDeparture" class="IptText300" />
						</dd>
						<dd>
							<div class="dd_left">目的地:</div>
								<input type="text" name="ticketArrival" id="ticketArrival" class="IptText300" />

						</dd>
						<dd>
							<div class="dd_left">车次:</div>
								<input type="text" name="ticketFlight" id="ticketFlight" class="IptText300" />
						...
						<dd>
							<div class="dd_left"></div>
							<div class="dd_right">
								<input type="submit" id="trainSubmit"
									class="button button-primary button-small"
									value="确定" /> <input type="button" id="trainCancel"
									class="button button-primary button-small"
									value="取消" />
							</div>
							<div class="clear"></div>
...
<script type="text/html" id="trainRow">
...
<td><input type="text" class='input-w80' name="itemDate"  class="Wdate"
								 /></td>
	<td><input type="text" class='input-w80' name="ticketDeparture" class="IptText300" /></td>
	<td><input type="text" class='input-w80' name="ticketArrival"  class="IptText300" /></td>
	<td><input type="text" class='input-w80' name="ticketFlight"  class="IptText300" /></td>
	<td><input type="text" class='input-w80' name="itemNum"  value="0"
								class="IptText300" /></td>
		<td><input type="text" class='input-w80' name="itemPrice"  value="0"
								class="IptText300" /></td>
		<td><input type="text" class='input-w80'  name="itemNumMinus" value="0"
								class="IptText300"></td>
		<td><input type="text" class='input-w80' name="itemTotal" value="0"
								class="IptText300" readonly="readonly"></td>
		<td><a name="del" href="javascript:void(0)">删除</a>
	</td>
</tr>
...
function showAdd(){
	$("#trainTblTr").append($("#trainRow").html());
}
function bindEvent(isAdd){
	$("#trainTblTr tr").each(function(){
		//绑定删除事件
		$(this).find("a[name='del']").unbind("click").bind("click",function(){
			var rowindex = $(this).closest("#trainTblTr").find("tr").index($(this).closest("tr")[0]);			
		...
		function calcSum(){
			var sum=0;
			//计算总价				
			$("#trainTblTr tr").find("input[name='itemTotal']").each(function(){
...
var layindex=0;
$(function(){
	 $("#groupDetail").load("<%=staticPath%>/booking/groupDetail.htm?gid="+$("#groupId").val()+"&stype=10");
	 if($("#flag").val()==1){
		 $("#btnSave").attr("style","display:none");
		 $("#btnSaveAdd").attr("style","display:none");
		 $("#train").attr("style","display:none");
		 $("#contact1").attr("style","display:none");
		 $("#trainBtn").attr("style","display:none");
						        ...
						        success: function(data) {
						            if (data.sucess ) {			  
						            	$.success("保存成功",function(){
						            		if(saveFrom=='saveadd'){
						            			refreshWindow("新增火车票订单","toAddTrainTicket?groupId="+data['groupId']);
						            		}else{
						            			refreshWindow("修改火车票订单","toAddTrainTicket?groupId="+data['groupId']+"&bookingId="+data['bookingId']+"&stateBooking="+data['stateBooking']);						            			
						            		}
						            	})
						            }else{
										$.error("操作失败"+data.msg);
	...
	function selectTrain(){
		layer.openSupplierLayer({
			title : '选择火车票',
			content : '<%=ctx%>/component/supplierList.htm?supplierType=10',//参数：操作类型（type:单选(single)、多选(multi)）供应商类型supplierType=1
...
function selectCashType(){
	...
	function delRowEvent(obj) {
		var rowindex = $(obj).closest("#trainTblTr").find("tr").index(
				$(obj).closest("tr")[0]);
