...
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
 <%@ include file="../../../../include/path.jsp" %>
          	...
          	<div class="in_tab mb-30">
          		...
          		<p class="in_tab_title"><b>保险订单</b></p>
          		<form id="bookingInsurance" action="">
          		<table class="in_tab_body" border="1" cellspacing="0" cellpadding="0">
 					...
 					<col width="8%" /><col width="8%" /><col width="" /><col width="6%" />
          			...
          			<tr>
						...
						<c:otherwise>
							<c:if test="${booking.stateBooking==0 }">保险未确认</c:if>
							<c:if test="${booking.stateBooking==1 }">保险已确认</c:if>
          				...
          				<td>
							...
							<a class="def" href="javascript:void(0)" onclick="newWindow('查看保险订单','<%=staticPath %>/booking/viewSupplier.do?groupId=${booking.groupId }&bookingId=${booking.id}')">查看</a>
						...
						<a class="def" href="javascript:void(0)" onclick="confirmState(this,${booking.id })">确认</a>
							...
							<c:if test="${optMap['EDIT'] && (booking.stateFinance==null || booking.stateFinance==0 ) }">
		         			<a class="def" href="javascript:void(0)" onclick="newWindow('修改保险订单','<%=staticPath %>/booking/toAddInsurance?groupId=${booking.groupId }&bookingId=${booking.id}')">修改</a>
							</c:if>
...
<script type="text/javascript">
	function del(obj,id,type){
		...
		$("#bookingInsurance").ajaxSubmit(options);
		}
	}
	function confirmState(obj,id){
		$.post("stateConfirm.do",{id:id},function(data){
	   		if(data.success){
	   			$(obj).hide();
	   			$(obj).parent().prev(".state").html("保险已确认");
	   			$.info('确认成功！');	   			
	   		}else{
	   			$.info(data.msg);
	   		}
