...
<body>
					...
					<dd>
						...
						<div class="dd_right">
							...
							<input type="hidden" name="supplierId" id="supplierId"
								...
								value="选择" onclick="selectInsurance()" id="insurance"/>
						</div>
						<div class="clear"></div>
					</dd>
					<dd>
							...
							<table cellspacing="0" cellpadding="0" class="w_table"
								...
								<thead>
									<tr>
										...
										<th><a href="javascript:void(0)" id="insuranceBtn"
											class="button button-primary button-small" >添加</a></th>

									</tr>
								</thead>
								<tbody id="insuranceTblTr">
											...
											<tr>
												<td><input type="hidden" name="detailId" value="${detail.id }"/>
														<select id="type1Id" name="type1Id" class="select-w80"> 
															<c:forEach items="${insuranceTypes}" var="t1" varStatus="vs">
	...
	<div id="addInsuranceItem" style="display: none;">
		<div class="p_container">
				...
				<form action="" id="detailForm">
					<dl class="p_paragraph_content" style="margin-top: 20px">
						...
						<dd>
							<div class="dd_left"></div>
							<div class="dd_right">
								<input type="submit" id="insuranceSubmit"
									class="button button-primary button-small"
									value="确定" /> <input type="button" id="insuranceCancel"
									class="button button-primary button-small"
									value="取消" />
							</div>
							<div class="clear"></div>
...
<script type="text/html" id="insuranceRow">
...
function showAdd(){
	$("#insuranceTblTr").append($("#insuranceRow").html());
}
function bindEvent(isAdd){
	$("#insuranceTblTr tr").each(function(){
		//绑定删除事件
		$(this).find("a[name='del']").unbind("click").bind("click",function(){
			var rowindex = $(this).closest("#insuranceTblTr").find("tr").index($(this).closest("tr")[0]);			
		...
		function calcSum(){
			var sum=0;
			//计算总价				
			$("#insuranceTblTr tr").find("input[name='itemTotal']").each(function(){
...
$(function(){
	<c:if test='${! empty bookingDetailList }'>
	 ...
	 $("#groupDetail").load("<%=staticPath%>/booking/groupDetail.htm?gid="+$("#groupId").val()+"&stype=15");
	 if($("#flag").val()==1){
		 $("#btnSave").attr("style","display:none");
		 $("#btnSaveAdd").attr("style","display:none");
		 $("#insurance").attr("style","display:none");
		 $("#contact1").attr("style","display:none");
		 $("#insuranceBtn").attr("style","display:none");
						        ...
						        success: function(data) {
						            if (data.sucess ) {		
						            	$.success("保存成功",function(){
						            		if(saveFrom=='saveadd'){
						            			refreshWindow("新增保险订单","toAddInsurance?groupId="+data['groupId']);
						            		}else{
						            			refreshWindow("修改保险订单","toAddInsurance?groupId="+data['groupId']+"&bookingId="+data['bookingId']+"&stateBooking="+data['stateBooking']);
						            		}
						            	})
						            }else{
										$.error("操作失败"+data.msg);
...
function selectCashType(){
	...
	function delRowEvent(obj) {
		var rowindex = $(obj).closest("#insuranceTblTr").find("tr").index(
				$(obj).closest("tr")[0]);
...
<script type="text/javascript">
    ...
    $(function(){
        var param = "";
        JAutocompleteEx("#supplierName", "<%=staticPath %>/tourGroup/getSupplier?supplierType=15", param, "supplierId", customerTicketCallback, 1);
    });
