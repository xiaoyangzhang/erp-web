...
<body>
	 <div class="p_container" >
	    <div class="p_container_sub" id="tab1">
	    	...
	    	<p class="p_paragraph_title"><b>其他支出信息</b></p>
	    	<dl class="p_paragraph_content">
	    		<dd>
	    			...
	    			<div class="dd_right">
	    				...
	    				<input type="button" class="button button-primary button-small" value="选择" onclick="selectSupplier()" id="supplier"/>
...
$(function(){	
	<c:if test='${! empty bookingDetailList }'>
	 ...
	 $("#groupDetail").load("<%=staticPath%>/booking/groupDetail.htm?gid="+$("#groupId").val()+"&stype=121");
	 if($("#flag").val()==1){
						        ...
						        success: function(data) {
						            if (data.sucess ) {
						            	$.success("保存成功",function(){
						            		if(saveFrom=='saveadd'){
						            			refreshWindow('新增其他支出','editOutcome.htm?groupId='+data['groupId']);
						            		}else{
						            			refreshWindow('修改其他支出','editOutcome.htm?groupId='+data['groupId']+"&bookingId="+data['bookingId']+"&stateBooking="+data['stateBooking']);
						            		}
						            	})
						            	}else{
										$.error("操作失败");
