...
<div class="p_container">
	<p class="p_paragraph_title" style="margin: 0;">
		<b>其他支出</b>
		<c:if test="${optMap['YDAP_OTHEROUT'] and groupCanEdit }">
			<a class="button button-primary button-small"
				href="javascript:void(0)"
				onclick="newWindow('新增其他支出','<%=ctx %>/booking/editOutcome.htm?groupId=${groupId }')">新增</a>&nbsp;&nbsp;&nbsp;</c:if>
		<a class="button button-primary button-small"
			...
			<tbody>
				<c:forEach items="${bookingInfo.bookingList  }" var="booking">
					<tr>
						...
						<td><fmt:formatNumber value="${booking.itemNum }"
								pattern="#.##" type="currency" /></td>
						...
						<td>
							<div class="tab-operate">
								...
								onclick="newWindow('查看支出','<%=staticPath %>/booking/viewSupplier.do?groupId=${booking.groupId }&bookingId=${booking.id}&supplierId=${booking.supplierId }') ">查看</a>
								<c:if
									test="${optMap['YDAP_OTHEROUT'] && (booking.stateFinance==null || booking.stateFinance==0 ) && booking.stateBooking==0}">
									<a class="def" href="javascript:void(0)"
										onclick="confirm(this,${booking.id })">确认</a>
								</c:if> <a class="def" href="javascript:void(0)"
								onclick="printOrder(${booking.id})">打印</a> <c:if
									test="${optMap['YDAP_OTHEROUT'] && (booking.stateFinance==null || booking.stateFinance==0 )}">
									<a class="def" href="javascript:void(0)"
										onclick="newWindow('修改支出','<%=staticPath %>/booking/editOutcome.htm?groupId=${booking.groupId }&bookingId=${booking.id}')">修改</a>
								</c:if> <c:if test="${optMap['YDAP_OTHEROUT'] && booking.canDelete==true}">
									<a class="def" href="javascript:void(0)"
