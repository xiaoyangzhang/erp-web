				...
				<c:forEach items="${bookingInfo.bookingList  }" var="booking">
					<tr>
						...
						<td>
							...
							onclick="newWindow('查看门票订单','<%=ctx %>/booking/viewSupplier.do?groupId=${booking.groupId }&bookingId=${booking.id}&supplierId=${booking.supplierId }')">查看</a>
							<c:if
							...
							onclick="printOrder(${booking.id})">打印</a> <c:if
								...
								<a class="def" href="javascript:void(0)"
									onclick="newWindow('修改门票订单','<%=ctx %>/booking/toAddSight?groupId=${booking.groupId }&bookingId=${booking.id}&supplierId=${booking.supplierId }')">修改</a>
