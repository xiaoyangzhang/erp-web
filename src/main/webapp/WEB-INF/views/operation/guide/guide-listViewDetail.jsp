					...
					<c:forEach items="${groupRequirements }" var="require">
						<tr>
		 					...
		 					<td>${require.language }</td>
	          				...
	          				<c:when test="${require.gender==0}">男</c:when><c:when test="${require.gender==1}">女</c:when>
				        ...
				        <c:if test="${i.count==1}">
				        	<td rowspan="${fn:length(group.guideTimes)}">${group.guide.guideName}<br/>${group.guide.guideMobile}</td>
	          				...
	          				<td rowspan="${fn:length(group.guideTimes)}">
	          					<%-- <a class="def" href="javascript:void(0)" onclick="newWindow('报账单','<%=staticPath %>/bookingGuide/finance.htm?bookingId=${group.guide.id}&groupId=${groupId}')">报账单</a> --%>
	          					 <c:if test="${groupMode==0}">
			                  		<a class="def" target="_blank"  href="previewGuideRoute.htm?id=${group.guide.id}&num=2">打印出团单</a>
			                  	 </c:if>
			                  	 <c:if test="${groupMode>0}">
			                  		<a class="def" target="_blank"  href="previewGuideRoute.htm?id=${group.guide.id}&num=1">打印出团单</a>
			                  	 </c:if>
	          				</td>
	          			 </c:if>	
	          			</tr>
