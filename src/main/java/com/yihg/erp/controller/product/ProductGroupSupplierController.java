...
//		model.addAttribute("groupSuppliers", groupSupplierService.selectProductGroupSuppliers(groupId));
		model.addAttribute("groupSuppliers", productFacade.selectProductGroupSuppliers(groupId));
...
//		return groupSupplierService.save(saveP) ==1?successJson():errorJson("操作失败！");
		ResultSupport resultSupport = productFacade.saveGroupSupplier(saveP);
...
//		return groupSupplierService.update(groupSupplier)==1?successJson():errorJson("操作失败！");
		ResultSupport resultSupport = productFacade.delSupplier(groupSupplier);
		return resultSupport.isSuccess() ? successJson() : errorJson("操作失败！");
	}
