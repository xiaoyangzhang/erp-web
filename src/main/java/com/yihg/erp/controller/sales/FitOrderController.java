...
//	private BookingSupplierService bookingSupplierService ;
	...
	@Autowired
	private FitOrderFacade fitOrderFacade;
	
	@RequestMapping(value = "getGroupPirceData.do", method = RequestMethod.GET)
	@ResponseBody
	public String getGroupPirceData(Integer productId, Integer supplierId,Date date) {
...
//		return string;
		...
		GetGroupPirceDataDTO getGroupPirceDataDTO=new GetGroupPirceDataDTO();
		getGroupPirceDataDTO.setDate(date);
		getGroupPirceDataDTO.setProductId(productId);
		getGroupPirceDataDTO.setSupplierId(supplierId);
		
		GetGroupPirceDataResult result=fitOrderFacade.getGroupPirceData(getGroupPirceDataDTO);
		Gson gson = new Gson();
		String string = gson.toJson(result.getPriceGroup());
...
//		model.addAttribute("lysfxmList", lysfxmList);
		...
		ToAddGroupOrderDTO toAddGroupOrderDTO=new ToAddGroupOrderDTO();
		toAddGroupOrderDTO.setCurBizId(WebUtils.getCurBizId(request));
		toAddGroupOrderDTO.setCurUser(curUser);
		toAddGroupOrderDTO.setDate(date);
		toAddGroupOrderDTO.setProductId(productId);
		
		ToAddGroupOrderResult result=fitOrderFacade.toAddGroupOrder(toAddGroupOrderDTO);
		
		model.addAttribute("supplierList", result.getSupplierList());
		model.addAttribute("vo", result.getVo());
		model.addAttribute("allowNum", result.getCount());
		...
		model.addAttribute("dicInfoCRList", result.getDicInfoCRList());
		model.addAttribute("dicInfoETList", result.getDicInfoETList());
...
//		model.addAttribute("lysfxmList", lysfxmList);
		...
		ToEditFirOrderDTO toEditFirOrderDTO=new ToEditFirOrderDTO();
		int bizId = WebUtils.getCurBizId(request);
		toEditFirOrderDTO.setBizId(bizId);
		toEditFirOrderDTO.setOrderId(orderId);
		
		ToEditFirOrderResult result=fitOrderFacade.toEditFirOrder(toEditFirOrderDTO);
...
//		model.addAttribute("orgUserJsonStr",platformEmployeeService.getComponentOrgUserTreeJsonStr(bizId));
		...
		ToFitOrderListDTO toFitOrderListDTO=new ToFitOrderListDTO();
		toFitOrderListDTO.setBizId(bizId);
		
		ToFitOrderListResult result=fitOrderFacade.toFitOrderList(toFitOrderListDTO);
...
//		pageBean = groupOrderService.selectFitOrderListPage(pageBean,WebUtils.getCurBizId(request),WebUtils.getDataUserIdSet(request),0);
...
//		}
//
//		GroupOrder order = groupOrderService.selectFitOrderTotalCount(groupOrder, WebUtils.getCurBizId(request),WebUtils.getDataUserIdSet(request),0);
//		model.addAttribute("page", pageBean);
//		model.addAttribute("totalOrder", order);
		
		GetFitOrdersDataDTO getFitOrdersDataDTO=new GetFitOrdersDataDTO();
		getFitOrdersDataDTO.setCurBizId(WebUtils.getCurBizId(request));
		getFitOrdersDataDTO.setGroupOrder(groupOrder);
		getFitOrdersDataDTO.setUserIdSet(WebUtils.getDataUserIdSet(request));
		
		GetFitOrdersDataResult result=fitOrderFacade.getFitOrdersData(getFitOrdersDataDTO);
		model.addAttribute("page", result.getPageBean());
		model.addAttribute("totalOrder", result.getOrder());
	}
	/**
	 * 散客订单打印预览
	 * @param request
	...
	public String getSubOrderListData(HttpServletRequest request,
		...
		GroupOrderListResult result=fitOrderFacade.getSubOrderListData(groupId);
...
//		model.addAttribute("tourGroup", tourGroup);
		...
		GetInsertFitGroupListDTO getInsertFitGroupListDTO=new GetInsertFitGroupListDTO();
		getInsertFitGroupListDTO.setBizId(WebUtils.getCurBizId(request));
		getInsertFitGroupListDTO.setTid(tid);
		getInsertFitGroupListDTO.setTourGroup(tourGroup);
		getInsertFitGroupListDTO.setUserIdSet(WebUtils.getDataUserIdSet(request));
		
		GetInsertFitGroupListResult result=fitOrderFacade.getInsertFitGroupList(getInsertFitGroupListDTO);
		model.addAttribute("page", result.getPageBean());
...
//		return successJson();
		...
		BaseStateResult result=fitOrderFacade.beforeInsertGroup(ids);
...
//		return successJson();
		...
		BaseStateResult result=fitOrderFacade.judgeMergeGroup(ids);
...
//		return successJson();
		...
		BaseStateResult result=fitOrderFacade.insertGroupMany(ids,code);
...
//		return successJson();
		...
		InsertGroupDTO insertGroup=new InsertGroupDTO();
		insertGroup.setCode(code);
		insertGroup.setId(id);
		insertGroup.setCurUserId(WebUtils.getCurUserId(request));
		insertGroup.setCurUserName(WebUtils.getCurUser(request).getName());
		
		BaseStateResult result=fitOrderFacade.insertGroup(insertGroup);
...
//		model.addAttribute("ids", ids);
		...
		GroupOrderListResult result=fitOrderFacade.toMergeGroup(ids);
		model.addAttribute("list", result.getOrders());
		model.addAttribute("ids", ids);
...
//				settingCommon.getMyBizCode(request),false);
//
//		return successJson();
		
		MergeGroupDTO mergeGroupDTO=new MergeGroupDTO();
		mergeGroupDTO.setMergeGroupOrderVO(mergeGroupOrderVO);
		mergeGroupDTO.setBizCode(bizSettingCommon.getMyBizCode(request));
		mergeGroupDTO.setBizId(WebUtils.getCurBizId(request));
		
		fitOrderFacade.mergeGroup(mergeGroupDTO);
...
//		model.addAttribute("page", pageBean);
		...
		ToImpNotGroupListResult result=fitOrderFacade.toImpNotGroupList(toImpNotGroupListDTO);
...
//		return successJson();
		...
		BaseStateResult result=fitOrderFacade.saveTransportInfo(saveFitOrderInfoDTO);
		if(result.isSuccess()){
			return successJson();
		}else{
			return errorJson(result.getError());
...
//		return successJson();
		...
		BaseStateResult result=fitOrderFacade.saveGuestInfo(saveFitOrderInfoDTO);
		if(result.isSuccess()){
			return successJson();
		}else{
			return errorJson(result.getError());
